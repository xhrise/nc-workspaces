...
/**
 * 原类名：KeywordRefDialog
 * @author zzl 2005-12-23
 */
public class KeywordRefDlg extends UfoDialog implements ActionListener {
     ...
     */
    public KeywordRefDlg(Container parent, KeyVO[] hasDefVos) {
        super(parent);
        if (hasDefVos == null)
            vos = new KeyVO[0];
        else
     ...
     * KeywordRefDialog 构造子注解。
     */
    public KeywordRefDlg(Dialog parent, KeyVO[] hasDefVos) {
        super(parent);
        if (hasDefVos == null)
            vos = new KeyVO[0];
        else
    ...
    private void initTableModel() {
        tablemodel = new KeyWordTable(CacheProxy
                .getSingleton().getKeywordCache().getAll(), vos);
