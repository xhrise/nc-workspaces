...
public class ClientEventHandler extends AbstractSPEventHandler{
	...
	protected void onBoRefresh() throws Exception {
     ...
     /**
		 * 弃审-付款单 添加弃审，弃审时检查操作单据制单月份是否结帐、成本凭证是否生成 modify by river for 2011-11-24
		 */
    @Override
    protected void onBoCancelAudit() throws Exception {
    	...
    	if((fkvo.getVbillstatus() == 1 || fkvo.getVbillstatus() == 0) && vapproveid.equals(pk_user)) {
				...
				if(Integer.valueOf(counts.toString()) == 0) {
					...
					if(validdlg.showModal() == UIDialog.ID_OK) {
						...
						if(obj == null || Integer.valueOf(obj.toString()) == 0) {
							try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark , validuser , validpass) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+fkvo.getPk_fk()+"' , '"+fkvo.getBillno()+"' , '认证失败，登录名不存在！' , '"+txtUser+"' , '"+txtPass+"')", null); } catch (Exception e) {};
						...
						if(obj == null) {
							try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark  , validuser , validpass , validcuserid) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+fkvo.getPk_fk()+"' , '"+fkvo.getBillno()+"' , '认证失败，密码错误！' , '"+txtUser+"' , '"+txtPass+"' , '"+cuserid+"')", null); } catch (Exception e) {e.printStackTrace();};
						...
						if(obj != null)
							if("ncca".equals(obj.toString().trim())) {
								...
								if(cadlg.showModal() == UIDialog.ID_OK) {
									...
									try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark , validuser , validpass , validcuserid) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+fkvo.getPk_fk()+"' , '"+fkvo.getBillno()+"' , '操作被取消' , '"+txtUser+"' , '"+txtPass+"' , '"+cuserid+"')", null); } catch (Exception e) {};
									return ;
								}
							}
						...
						if(obj == null || Integer.valueOf(obj.toString()) == 0) {
							...
							try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark , validuser , validpass , validcuserid) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+fkvo.getPk_fk()+"' , '"+fkvo.getBillno()+"' , '注意：该用户没有配置按钮权限！ ' , '"+txtUser+"' , '"+txtPass+"' , '"+cuserid+"')", null); } catch (Exception e) {};
						...
						try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark  , validuser , validpass ) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+fkvo.getPk_fk()+"' , '"+fkvo.getBillno()+"' , '操作被取消')", null); } catch (Exception e) {};
						return ;
			...
			if(JOptionPane.showConfirmDialog((ClientUI)getBillUI(), "确定要执行弃审操作吗？ ", "提示", JOptionPane.YES_NO_OPTION) == 0) {
				...
				sql.append(" values('"+fkvo.getPk_corp()+"' , '"+fkvo.getBillno()+"' , '"+fkvo.getPk_fk()+"' , to_char(sysdate , 'yyyy-mm-dd HH24-mi-ss') , 'eh_arap_fk' , '付款单' , '"+pk_user+"' , 'TRUE')");
				try {
				...
				StringBuffer sql = new StringBuffer();
				sql.append(" insert into eh_unapprove (pk_corp , vbillno , pk_bill , ts,billtablename , billname , OPERATER , REMARK) ");
				sql.append(" values('"+fkvo.getPk_corp()+"' , '"+fkvo.getBillno()+"' , '"+fkvo.getPk_fk()+"' , to_char(sysdate , 'yyyy-mm-dd HH24-mi-ss') , 'eh_arap_fk' , '付款单' , '"+pk_user+"' , 'FALSE')");
				try {
					iUAPQueryBS.executeQuery(sql.toString(), null);
				} catch(Exception e) {}
			}
