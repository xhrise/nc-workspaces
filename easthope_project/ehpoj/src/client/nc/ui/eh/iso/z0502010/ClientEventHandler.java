 		...
 		// 添加DIALOG add by river for 2011-12-22
 					...
 					if(validdlg.showModal() == UIDialog.ID_OK) {
 							...
 							if("ncca".equals(obj.toString().trim())) {
 								...
 								if(cadlg.showModal() == UIDialog.ID_OK) {
									 ...
									 */
 									...
 									try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark , validuser , validpass , validcuserid) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+scbvo.getPk_receipt()+"' , '"+scbvo.getBillno()+"' , '操作被取消' , '"+txtUser+"' , '"+txtPass+"' , '"+cuserid+"')", null); } catch (Exception e) {};
 									return ;
 								}
 							}
 						...
 						if(obj == null || Integer.valueOf(obj.toString()) == 0) {
 							...
 							try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark , validuser , validpass , validcuserid) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+scbvo.getPk_receipt()+"' , '"+scbvo.getBillno()+"' , '注意：该用户没有配置按钮权限！ ' , '"+txtUser+"' , '"+txtPass+"' , '"+cuserid+"')", null); } catch (Exception e) {};
 							throw new Exception("认证失败，用户无此权限！ ");
 						
 						}
 						
 						try {iUAPQueryBS.executeQuery("update eh_validoperate set opeatefield = 'Y' where  user_password = '"+txtPass+"' ))and pk_button = (select pk_button from eh_buttons where buttonname = '弃审') and dr = 0", null); } catch (Exception e) {}
 						
 						try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark  , validuser , validpass , validcuserid) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+scbvo.getPk_receipt()+"' , '"+scbvo.getBillno()+"' , 'TRUE' , '"+txtUser+"' , '"+txtPass+"' , '"+cuserid+"')", null); } catch (Exception e) {};
 						
 					} else {
 						try { iUAPQueryBS.executeQuery("insert into eh_validoperate_log( pk_log , cuserid , pk_corp , operatedate , type , pk_bill , billno , remark  , validuser , validpass ) values (generatepk('"+pk_corp+"') , '"+pk_user+"' , '"+pk_corp+"' , '"+ClientUI.getCE().getDate().toString()+"' , 'CancelAudit' , '"+scbvo.getPk_receipt()+"' , '"+scbvo.getBillno()+"' , '操作被取消')", null); } catch (Exception e) {};
 						return ;
 			...
 			if(JOptionPane.showConfirmDialog((ClientUI)getBillUI(), "确定要执行弃审操作吗？ ", "提示", JOptionPane.YES_NO_OPTION) == 0) {
		 		...
		 		sql.append(" values('"+scbvo.getPk_corp()+"' , '"+scbvo.getBillno()+"' , '"+scbvo.getPk_receipt()+"' , to_char(sysdate , 'yyyy-mm-dd HH24-mi-ss') , 'eh_stock_checkreport' , '检测报告单', '"+pk_user+"' , 'TRUE')");
		 		try {
 				...
 				StringBuffer sql = new StringBuffer();
 	 			sql.append(" insert into eh_unapprove (pk_corp , vbillno , pk_bill , ts,billtablename , billname ,  OPERATER , REMARK) ");
 	 			sql.append(" values('"+scbvo.getPk_corp()+"' , '"+scbvo.getBillno()+"' , '"+scbvo.getPk_receipt()+"' , to_char(sysdate , 'yyyy-mm-dd HH24-mi-ss') , 'eh_stock_checkreport' , '检测报告单', '"+pk_user+"' , 'FALSE')");
 	 			try {
 	 				iUAPQueryBS.executeQuery(sql.toString(), null);
 	 			} catch(Exception e) {}
 			}
