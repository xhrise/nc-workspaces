 ...
 */
public class SystemBaseDLG extends AbstractReferQueryUI implements ActionListener,
		BillEditListener, BillTableMouseListener, ListSelectionListener {
	protected BillListPanel ivjbillListPanel = null;

	private JPanel ivjUIDialogContentPane = null;

	private UIButton ivjbtnCancel = null;//取消按钮 houcq添加注释

	private UIButton ivjbtnOk = null;//确定按钮houcq添加注释
	
	private UIButton selectall = null;//生成凭证按钮houcq添加注释
	
	private UIButton cancelselect = null;//全消按钮houcq添加注释

	private UIButton ivjbtnQuery = null;//查询按钮houcq添加注释

	private UIPanel ivjPanlCmd = null;
	
	private UIButton ivjbtnSelectAll = null;//全选按钮 houcq增加
	 ...
	 * @param parent
	 */
	public SystemBaseDLG(String pkField, String pkCorp, String operator, String funNode,
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == getbtnOk()) {
			...
			onQuery();
		} else if (e.getSource() == getbtnGenVoucher()) {
			onBoGenVoucher();
		}else if (e.getSource() == getbtnSelectAll()) {
			onSelectAll();//add by houcq 
		}
	...
	public void addListenerEvent() {
		...
		getbtnGenVoucher().addActionListener(this);
		getbtnCancelSelect().addActionListener(this);
		getbtnSelectAll().addActionListener(this);//add by houcq
		//表头列表 行切换事件处理器
	...
	private UIButton getbtnGenVoucher() {
		if (selectall == null) {
			selectall = new UIButton();
			selectall.setName("selectAll");
			selectall.setText("生成凭证");
		}
		return selectall;
	}
	...
	private UIButton getbtnCancelSelect() {
		if (cancelselect == null) {
			...
			//cancelselect.setText("反选"); //houcq注释
			cancelselect.setText("全消");//add by houcq
		}
		return cancelselect;
	}
	...
	//getbtnSelectAll() add by houcq 
	private UIButton getbtnSelectAll() {
		if (ivjbtnSelectAll == null) {
			ivjbtnSelectAll = new UIButton();
			ivjbtnSelectAll.setName("ivjbtnSelectAll");
			ivjbtnSelectAll.setText("全选");
		}
		return ivjbtnSelectAll;
	}
	/**
	 * 主表查询条件
	 * @return
	...
	private UIPanel getPanlCmd() {
		if (ivjPanlCmd == null) {
			...
			//add by houcq end
			ivjPanlCmd.add(getbtnGenVoucher(), getbtnGenVoucher().getName());
//			ivjPanlCmd.add(getbtnCancelSelect(), getbtnCancelSelect().getName());
...
//		  }
	...
	public void onSelectAll() {
		...
		for(int i=0;i<headrow;i++){
			//add by houcq begin
			getbillListPanel().getBillListData().getHeadBillModel().setRowState(i, BillModel.SELECTED);
			//add by houcq end
			/*houcq注释
	...
	public void onCancleSelectAll() throws Exception {
		...
		for(int i=0;i<headrow;i++){
			//add by houcq begin
			getbillListPanel().getBillListData().getHeadBillModel().setRowState(i, BillModel.UNSTATE);
			//add by houcq end
			/* houcq 注释
			int rowstate = getbillListPanel().getHeadBillModel().getRowState(i);
			if(rowstate!=BillModel.SELECTED){
