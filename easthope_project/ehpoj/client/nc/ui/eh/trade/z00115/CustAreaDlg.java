...
import nc.ui.eh.refpub.CustAreaclRefModel;
...
public class CustAreaDlg extends UIDialog implements 
	BillEditListener {
	 ...
	 * @param owner 父面板
	 */
	public CustAreaDlg(Container parent) {
		super(parent);
		initialize();
	}
	...
	public BillCardPanel getBillCardPanel(){
		if(ivjBillCardPanel == null){
			...
			BillTempletVO vo = ivjBillCardPanel.getTempletData("ZB50",null,null,null,null);
			BillData billDataVo = new BillData(vo);
			ivjBillCardPanel.setBillData(billDataVo);
			BillItem areaItem = ivjBillCardPanel.getHeadItem("oldpk_areacl");
	...
	public void afterEdit(BillEditEvent e) {
		...
		if(e.getKey().equals("oldpk_areacl")){
			BillItem oldItem = getBillCardPanel().getHeadItem("oldpk_areacl");
	        ...
	        try {
				   ...
				   BillItem custItem = getBillCardPanel().getHeadItem("pk_cumandoc");
			        ...
			        CustAreaclRefModel  ref=new CustAreaclRefModel();
			        custRef.setRefModel(ref);
			        custRef.setMultiSelectedEnabled(true);
			        custRef.setWhereString("pk_areacl in "+pkss);
	           }catch (Exception e1) {
					  e1.printStackTrace();
			}
