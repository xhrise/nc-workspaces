...
//				m_noteVOs = PfWorkflowBO_Client.queryCheckNote(billcardpanel.getHeadItem(pk_code).getValueObject().toString());
	...
	public String[] getItemValuesByExpress(String itemExpress) {
		...
		int rowCount  = m_billcardpanel.getBillModel("range").getRowCount()+
                        m_billcardpanel.getBillModel("polic").getRowCount()+m_billcardpanel.getBillModel("checkinv").getRowCount();
		try {
...
//			}
			//表体
            //range二次折扣表体第一个页签名称 add by zqy 2008-9-27 17:39:58
			else{
                
                bodyCount = m_billcardpanel.getBillModel("range").getBodyItems().length;
                for (int i = 0; i < bodyCount; i++){
                    BillItem item = m_billcardpanel.getBillModel("range").getBodyItems()[i];
                    if(item==null) return null;
                    String[] rslt = new String[rowCount];
                    if (item.getKey().equals(itemExpress)) {
                        ...
                        if(item.getDataType()==4){
                            for (int j = 0; j < rowCount; j++) {
                                if(m_billcardpanel.getBillModel("range").getValueAt(j,  item.getKey())==null){
                                    rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                }else{
                                    if(m_billcardpanel.getBillModel("range").getValueAt(j,  item.getKey()).toString().equals("false")){
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                    }else{
                        ...
                        //UIRefPane or UICombox
                            ...
                            for (int j = 0; j < rowCount; j++) {
                                rslt[j] = m_billcardpanel.getBillModel("range").getValueAt(j,  item.getKey())==null?" ":m_billcardpanel.getBillModel("range").getValueAt(j,  item.getKey()).toString();
                ...
                //XXXXXXXNewyear 
                //polic为二次折扣表体的第二个页签名称
                bodyCount = m_billcardpanel.getBillModel("polic").getBodyItems().length;
                for (int i = 0; i < bodyCount; i++){
                    BillItem item = m_billcardpanel.getBillModel("polic").getBodyItems()[i];
                    if(item==null) return null;
                    String[] rslt = new String[rowCount];
                    if (item.getKey().equals(itemExpress)) {
                        //UICheckbox
                        if(item.getDataType()==4){
                            for (int j = 0; j < rowCount; j++) {
                                if(m_billcardpanel.getBillModel("polic").getValueAt(j,  item.getKey())==null){
                                    rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                }else{
                                    if(m_billcardpanel.getBillModel("polic").getValueAt(j,  item.getKey()).toString().equals("false")){
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                    }else{
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000164")/*@res "是"*/;
                                    }
                        ...
                        //UIRefPane or UICombox
                            ...
                            for (int j = 0; j < rowCount; j++) {
                                rslt[j] = m_billcardpanel.getBillModel("polic").getValueAt(j,  item.getKey())==null?
                                        " ":m_billcardpanel.getBillModel("polic").getValueAt(j,  item.getKey()).toString();
                                //rslt[j] = m_billcardpanel.getBodyValueAt(j, item.getKey()) == null ? "" : m_billcardpanel.getBodyValueAt(j, item.getKey()).toString();
                            }
                        }
                        return rslt;
                ...
                //客户结算产品页签
                bodyCount = m_billcardpanel.getBillModel("checkinv").getBodyItems().length;
				for (int i = 0; i < bodyCount; i++){
                    BillItem item = m_billcardpanel.getBillModel("checkinv").getBodyItems()[i];
                    if(item==null) return null;
                    String[] rslt = new String[rowCount];
                    if (item.getKey().equals(itemExpress)) {
                        //UICheckbox
                        if(item.getDataType()==4){
                            for (int j = 0; j < rowCount; j++) {
                                if(m_billcardpanel.getBillModel("checkinv").getValueAt(j,  item.getKey())==null){
                                    rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                }else{
                                    if(m_billcardpanel.getBillModel("checkinv").getValueAt(j,  item.getKey()).toString().equals("false")){
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                    }else{
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000164")/*@res "是"*/;
                                    }
                        ...
                        //UIRefPane or UICombox
                            ...
                            for (int j = 0; j < rowCount; j++) {
                                rslt[j] = m_billcardpanel.getBillModel("checkinv").getValueAt(j,  item.getKey())==null?" ":m_billcardpanel.getBillModel("checkinv").getValueAt(j,  item.getKey()).toString();
                                //rslt[j] = m_billcardpanel.getBodyValueAt(j, item.getKey()) == null ? "" : m_billcardpanel.getBodyValueAt(j, item.getKey()).toString();
                            }
                        }
                        return rslt;
