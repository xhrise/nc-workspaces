...
public class ClientUI extends nc.ui.ic.pub.bill.GeneralBillClientUI {
  ...
  public void afterInvEdit(nc.ui.pub.bill.BillEditEvent e) {
    ...
    try {
      ...
      if (e.getValue().toString().trim().length() == 0) {
        ...
        // 同步vo
        if (getM_voBill() != null) {
          getM_voBill().setItemValue(row, getEnvironment().getShouldNumItemKey(), null);
          getM_voBill().setItemValue(row, getEnvironment().getShouldAssistNumItemKey(), null);
        }
        if (getBillCardPanel().getBodyItem(getEnvironment().getShouldNumItemKey()) != null)
          getBillCardPanel().setBodyValueAt(null, row, getEnvironment().getShouldNumItemKey());
        if (getBillCardPanel().getBodyItem(getEnvironment().getShouldAssistNumItemKey()) != null)
          getBillCardPanel().setBodyValueAt(null, row, getEnvironment().getShouldAssistNumItemKey());
        // 表尾
        setTailValue(null);
      }
      else {
        ...
        if (sTempID1 != null && sTempID1.trim().length() != 0) {
          ...
          if (getBillCardPanel().getHeadItem(IItemKey.WAREHOUSE) != null)
            sTempID2 = getBillCardPanel().getHeadItem(IItemKey.WAREHOUSE)
                .getValue();
          ...
          if (getM_voBill() != null) {
            ...
            getM_voBill().setItemValue(row, IItemKey.NAME_HEADID,
  ...
  protected void execExtendFormula(ArrayList alListData) {
    ...
    nc.ui.scm.pub.billutil.ClientCacheHelper.getColValue(headVO, new String[] {
      "pk_cubasdoctran"
    }, "dm_trancust", "pk_trancust", new String[] {
      "pkcusmandoc"
    }, "cwastewarehouseid");
...
//  }
  ...
  protected nc.ui.ic.auditdlg.ClientUIInAndOut getDispenseDlg(String sTitle,
    ...
    m_dlgInOut.setVO(getM_voBill(), alInVO, alOutVO, getBillType(),
        getM_voBill().getPrimaryKey().trim(), getEnvironment().getCorpID(), getEnvironment().getUserID());
  ...
  public void initialize() {
    ...
    try {
      ...
      getBillCardPanel().getBodyPanel().getTable().setSelectionMode(
          javax.swing.ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
      getBillListPanel().getChildListPanel().getTable().setSelectionMode(
          javax.swing.ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
    }
    catch (Exception e) {
  ...
  protected void initPanel() {
    ...
    // “形成代管”按钮
    getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_XCDG).setEnabled(
        false);
    // “配套”按钮
    ...
    try {
      ...
      if (getM_iCurPanel() == BillMode.List) { // 列表形式下
        if (getM_alListData() != null && getM_iLastSelListHeadRow() >= 0
          ...
          sBusitypeid = (String) voMyBill.getHeaderValue(m_sBusiTypeItemKey);
        }
      }
      else { // 表单
      ...
      if (sBusitypeid != null && m_alBrwLendBusitype == null) {
        ...
        m_alBrwLendBusitype = (ArrayList) GeneralBillHelper.queryInfo(
            new Integer(QryInfoConst.QRY_BRW_LEND_BIZTYPE), alParam);
  ...
  protected void onAdd4331() {
    try {
      ...
      sourecVO[0] = new PubTransBillPaneVO("4331","发货单",
          (ISourceRedunUI)Class.forName("nc.ui.so.soreceive.redun.ReceiveTo4CRedunSourceCtrl").newInstance(),
          null,"SO",null,null);
      
      PubTransBillPaneVO[] curVO = new PubTransBillPaneVO[1];
      curVO[0] = new PubTransBillPaneVO("4C","销售出库单",
  ...
  protected class ButtonManager211 extends nc.ui.ic.pub.bill.GeneralButtonManager {

	public ButtonManager211(GeneralBillClientUI clientUI) throws BusinessException {
		super(clientUI);
	}
      ...
      if(bo.getTag()!=null && bo.getTag().indexOf(":")>0){
        String billtype = bo.getTag().substring(0, bo.getTag().indexOf(":"));
        if(billtype.equals("4331")){
          onAdd4331();
          return;
        }
      }
	    if (bo == getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_XCDG))
	      // 处理"形成待管"
	      onFormMemo();
	    else if (bo == getButtonManager().getButton(
	        ICButtonConst.BTN_ASSIST_FUNC_DISPENSE))
	      onDispense();
	    else if (bo == getButtonManager().getButton(ICButtonConst.BTN_PRINT_SUM_BATCH))
	      onPrintLot();
	    else if (bo == getButtonManager().getButton(
	      ...
	      onRefInBill();
	    else if (bo == getButtonManager().getButton(ICButtonConst.BTN_RUSH_QUERY))
	      onRushQuery();
	    else if (bo == getButtonManager().getButton(ICButtonConst.BTN_OUT_RETURN))
	      onOutReturn();
	    else if (bo == getButtonManager().getButton(
		        ICButtonConst.BTN_ASSIST_FUNC_REFER_DIRECTIN))
		      onRefDirectInBill();
	    else if(bo == getButtonManager().getButton(ICButtonConst.BTN_ASSIST_COOP_45))
	    	onCoop45Save();
	    else {
...
//	    }
	   ...
	   * nc.ui.ic.pub.bill.GeneralBillClientUI.setAlistDataByFormula(int, ArrayList)
	   * 执行了表头、表体公式，所以切不可在代码中重复执行以下代码：
...
//					str_where = str_where.replace("head.dbilldate.from", "head.dbilldate");
//				if(str_where.indexOf("head.dbilldate.end") > -1)
//					str_where = str_where.replace("head.dbilldate.end", "head.dbilldate");
...
//	      }
//	      
//	      voCond.setParam(QryConditionVO.QRY_LOGCORPID, getEnvironment().getCorpID());
//	      voCond.setParam(QryConditionVO.QRY_LOGUSERID, getEnvironment().getUserID());
//
...
//	          getBillType(), voCond);
	      ...
	      try {
	        ...
	        timer.showExecuteTime("@@setAlistDataByFormula公式解析时间：");/*-=notranslate=-*/
	      ...
	      if (alListData != null && alListData.size() > 0) {
	        ...
	        if (m_iBillQty > 0)
	          showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	              "4008busi", "UPP4008busi-000290", null, new String[] {
	                (new Integer(m_iBillQty)).toString()
	              })/* @res "共查到{0}张单据！" */);
	        else
	          showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	  ...
	  public boolean onSave() {
	    ...
	    // 在蓬威石化做销售出库保存时判断表体项目字段是否为空。
	    // add by river for 2012-08-13
	    // start ..
	    if("1135".equals(getCorpPrimaryKey())) {
		    if(getBillVO() != null && getBillVO().getChildrenVO() != null && getBillVO().getChildrenVO().length > 0) {
		    	for(CircularlyAccessibleValueObject cavo : getBillVO().getChildrenVO()) {
		    		Object cprojectid = cavo.getAttributeValue("cprojectid");
		    		if(cprojectid == null || "".equals(cprojectid)) {
		    			
		    			showErrorMessage("表体 [ 项目 ] 不能为空");
		    			return false;
		    		}
	  ...
	  private void onFormMemo() {
	    if (getM_iLastSelListHeadRow() >= 0 && getM_alListData() != null
	        && getM_alListData().size() > getM_iLastSelListHeadRow()
	        && getM_alListData().get(getM_iLastSelListHeadRow()) != null) {
	      if (((GeneralBillVO) getM_alListData().get(getM_iLastSelListHeadRow()))
	      ...
	      voBill.setHeaderValue("coperatorid", getEnvironment().getUserID());
	  ...
	  private void onDispense() {
	    ...
	    if (getBillCardPanel().getBillTable().getSelectedRows().length >= 1) {
	      ...
	      if (nc.ui.pub.beans.UIDialog.ID_CANCEL == nc.ui.pub.beans.MessageDialog
	          .showOkCancelDlg(getClientUI(), null, nc.ui.ml.NCLangRes.getInstance()
	              .getStrByID("4008busi", "UPP4008busi-000268")/*
	                                                             * @res
	      ...
	      voBill.getHeaderVO().setDbilldate(new nc.vo.pub.lang.UFDate(getEnvironment().getLogDate()));
	      ...
	      if (m_dlgInOut.isOK()) {
	        try { // 更新表尾
	          ...
	          setDispenseFlag((GeneralBillVO) ((GeneralBillVO) getM_alListData()
	              .get(getM_iLastSelListHeadRow())), rownums);
	          setM_voBill((GeneralBillVO) ((GeneralBillVO) getM_alListData().get(
	              getM_iLastSelListHeadRow())).clone());
	          ...
	          nc.ui.pub.beans.MessageDialog
	              .showErrorDlg(getClientUI(), null, e.getMessage());
	        }
	      }
	    }
	  }
	  ...
	  public void onPrintLot() {
	    ...
	    try {
	      // 调出打印窗口
	      // 依当前是列表还是表单而定打印内容
	      if (getM_iMode() == BillMode.Browse && getM_iCurPanel() == BillMode.Card) { // 浏览
	        ...
	        if (getM_iLastSelListHeadRow() != -1 && null != getM_alListData()
	          ...
	          vo = (GeneralBillVO) getM_alListData()
	              .get(getM_iLastSelListHeadRow());
	          if (getBillCardPanel().getHeadItem("vcustname") != null)
	            vo.setHeaderValue("vcustname", getBillCardPanel().getHeadItem(
	                "vcustname").getValue());
	        }
	        ...
	        if ((null == vo.getChildrenVO()) || (vo.getChildrenVO().length == 0)
	        ...
	        dvomerger.setSummingAttr(new String[] {
	            "nshouldoutnum", "nshouldoutassistnum", "noutnum", "noutassistnum",
	            "nmny"
	        });
	        ...
	        for (int i = 0; i < alBill.size(); i++) {
	          ...
	          dvomerger.setSummingAttr(new String[] {
	              "nshouldoutnum", "nshouldoutassistnum", "noutnum",
	              "noutassistnum", "nmny"
	          });
	          nc.vo.ic.pub.bill.GeneralBillItemVO[] itemvosnew = (nc.vo.ic.pub.bill.GeneralBillItemVO[]) dvomerger
	              .mergeByGroup(gvobak.getItemVOs());
	        ...
	        showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("4008busi",
	            "UPP4008busi-000249")/* @res "只能在浏览状态下打印" */);
	    }
	    catch (Exception e) {
	      SCMEnv.error(e);
	      showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("4008busi",
	          "UPPSCMCommon-000061")/* @res "打印出错" */
	          + e.getMessage());
	    }
	  }
	  ...
	  private void onPrintCert() {
	    ...
	    try {
	      if (!m_isCheckQCstartup) {
	        m_isQCstartup = nc.ui.ic.pub.tools.GenMethod.isProductEnabled(
	            getEnvironment().getCorpID(), nc.vo.pub.ProductCode.PROD_QC);
	        m_isCheckQCstartup = true;
	      }
	      if (!m_isQCstartup) {
	        showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	            "4008other", "UPP4008other-000492")/* @res "质量管理没有启用！" */);
	        return;
	      }
	    ...
	    else if (getM_iLastSelListHeadRow() != -1 && null != getM_alListData()
	        && getM_alListData().size() != 0) {
	      voBill = (GeneralBillVO) getM_alListData()
	          .get(getM_iLastSelListHeadRow());
	      // ((IQcToIc_CertService) obj).printCert(this, voBill, client);
	      ((CertService) obj).printCert(getClientUI(), voBill, client);
	    }
	  ...
	  private void onAdd4L() {
	    ...
	    try {
	      if (getM_iCurPanel() == BillMode.List) {
	        // 列表界面
	        int seleHeadRow = getBillListPanel().getHeadTable().getSelectedRow();
	        if (seleHeadRow < 0) {
	          showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	              "40080802", "UPT40080802-000310")/* @res "请先选择一张单据和要生成组装单的成套件" */);
	          return;
	        }
	        /*
	         * billVO4C = (GeneralBillVO) getBillListPanel().getSelectedVO(
	        ...
	        if (seleBodyRowCount != 1)
	          return;
	        // 获得所选行号
	        int seleBodyRow = getBillListPanel().getBodyTable().getSelectedRow();
	        if (seleBodyRow > -1) {
	          // 根据行号获得所选行VO
	          if (null != billVO4C.getChildrenVO()
	              && billVO4C.getChildrenVO().length > 0)
	            item4C = (GeneralBillItemVO) billVO4C.getChildrenVO()[seleBodyRow];
	        }
	      }
	      else {
	        ...
	        if (null == billVO4C.getHeaderValue("cwarehousename")) {
	          showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	              "40080802", "UPT40080802-000318")/* @res "请先选择表头仓库" */);
	          return;
	        }
	        int seleBodyRow = getBillCardPanel().getBodyPanel().getTable()
	            .getSelectedRow();
	        if (seleBodyRow > -1) {
	          ...
	          showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	              "40080802", "UPT40080802-000312")/* @res "请选择存货为成套件的行" */);
	          return;
	        }
	      }
	      ...
	      if (null != sInvbasID && !"".equals(sInvbasID.trim())) {
	        ...
	        nc.itf.uap.bd.inv.IInventoryQry invQry = (nc.itf.uap.bd.inv.IInventoryQry) nc.bs.framework.common.NCLocator
	            .getInstance().lookup(
	                nc.itf.uap.bd.inv.IInventoryQry.class.getName());
	        if (null != invQry) {
	          ...
	          if (null != sSetpartsflag && !sSetpartsflag.booleanValue()) {
	            showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	                "40080802", "UPT40080802-000314")/* @res "选定的存货不是成套件" */);
	            // 如果不是成套件则返回
	            return;
	          }
	          if (null != billVO4C) {
	            // 准备数据
	            billVO4C.setChildrenVO(new GeneralBillItemVO[] {
	              item4C
	            });
	            try {
	              // 如果是退回、退货操作检查表体数量是否有负数，有的话报错
	            ...
	            if (null == specBillVO4L) {
	              showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	                  "40080802", "UPT40080802-000316")/* @res "数据交换失败" */);
	              return;
	            }

	            RefMsg msg = new RefMsg(this);
	            msg.setBillVos(new SpecialBillVO[] {
	              specBillVO4L
	            });
	            msg.setIBillOperate(ILinkType.LINK_TYPE_ADD);
	            // this.setVisible(false);
	            nc.ui.ic.ic231.ClientUI.openNodeAsDlg(getClientUI(), msg);
	          }
	        }
	      }
	    }
	      ...
	      // 日志异常
	      nc.vo.scm.pub.SCMEnv.out(e);
	      showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("40080802",
	          "UPT40080802-000320")/* @res "显示组装单界面出错：" */
	          + e.getMessage());
	    }
	  ...
	  protected void onRefInBill() {
	    try {
	      QryInBillDlg dlgBill = new QryInBillDlg("cgeneralhid", getEnvironment().getCorpID(),
	          getEnvironment().getUserID(), "40089907", "1=1", "45", null, null, "4C", getClientUI());

	      if (dlgBill == null)
	        return;

	      nc.ui.ic.pub.pf.ICBillQuery dlgQry = new nc.ui.ic.pub.pf.ICBillQuery(getClientUI());
	      // 加载源查询模版
	      // dlgQry.setParent(this);
	      dlgQry.setTempletID(getEnvironment().getCorpID(), "40080608", getEnvironment().getUserID(), null, "40089907");
	      dlgQry.initData(getEnvironment().getCorpID(), getEnvironment().getUserID(), "40089907", null, "4C", "45", null);
	      //修改人：刘家清 修改时间：2008-9-22 下午04:59:35 修改原因：业务类型按非直运过滤。
	      dlgQry.setRefInitWhereClause("head.cbiztype","业务类型"," verifyrule != 'Z' and (pk_corp='"+getEnvironment().getCorpID()+"' or pk_corp='@@@@') ",null);
	      if (dlgQry.showModal() == nc.ui.pub.beans.MessageDialog.ID_OK) {
	        ...
	        sWhere.append(" and head.cbiztype in (select pk_busitype from bd_busitype where verifyrule != 'Z' and (pk_corp='"+getEnvironment().getCorpID()+"' or pk_corp='@@@@')) ");

	        // 加载源参照对话框
	        dlgBill.initVar("cgeneralhid", getEnvironment().getCorpID(), getEnvironment().getUserID(), null, sWhere
	            .toString(), "45", null, null, "4C", null, this);
	        ...
	        if (dlgBill.showModal() == nc.ui.pub.beans.MessageDialog.ID_OK) {
	          ...
	          if (voRetvos != null && voRetvos.length > 0)
	            cbiztype = (String) voRetvos[0].getParentVO().getAttributeValue(
	                "cbiztype");
 ...
 */
		    ...
		    try {
		      ...
		      if (dlgQry.showModal() == nc.ui.pub.beans.MessageDialog.ID_OK) {
		        ...
		        if (dlgBill.showModal() == nc.ui.pub.beans.MessageDialog.ID_OK) {
		          ...
		          if (vos == null || 0 == vos.length || null == vos[0].getChildrenVO()) {
		            nc.vo.scm.pub.ctrl.GenMsgCtrl
		                .printHint("qrybilldlg getRetVos null");
		            return;
		          }
		          ...
		          voRetvos = (GeneralBillVO[])GenMethod.callICService("nc.bs.ic.ic211.GeneralHBO","fillDirectSaleOrderInfo",new Class[] { voRetvos.getClass() },new Object[] {(GeneralBillVO[])voRetvos});
		          for(GeneralBillVO voRetvo :(GeneralBillVO[])voRetvos)
		        	  voRetvo.getHeaderVO().setBdirecttranflag(UFBoolean.TRUE);
		          // 控制界面
		          String cbiztype = null;
		          if (voRetvos != null && voRetvos.length > 0)
		            cbiztype = (String) voRetvos[0].getParentVO().getAttributeValue(
	  ...
	  /**
	   * 销售出库对冲联查 v52。 功能： 参数： 返回： 例外： 日期：(2002-04-18 10:43:46) 修改日期，修改人，修改原因，注释标志：
	   */
	  private void onRushQuery() {
	    try {
	      if (!GenMethod.isProductEnabled(getEnvironment().getCorpID(), ProductCode.PROD_SO)) {
	        showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	            "4008busi", "UPP4008busi-000069")/* @res "销售产品没有启用" */);
	        return;
	      }
	      if (BillMode.New == getM_iMode() || BillMode.Update == getM_iMode()) {
	        showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID(
	            "4008other", "UPP4008other-000506")/* @res "请单据保存之后再查询！" */);
	        return;
	      }
	      ...
	      if (BillMode.List == getM_iCurPanel()) {
	        ...
	        cgeneralhid = (String) getBillListPanel().getHeadBillModel()
	            .getValueAt(getBillListPanel().getHeadTable().getSelectedRow(),
	                IItemKey.CGENERALHID);
	      }
	      else {
	        if (getM_voBill() != null && getM_voBill().getHeaderVO() != null)
	          cgeneralhid = getM_voBill().getHeaderVO().getCgeneralhid();
	      }
	      ...
	      RushLinkQueryVO[] rushvos = (RushLinkQueryVO[]) GenMethod
	          .callICEJBService("nc.bs.ic.ic211.GeneralHBO",
	              "queryRushLinkQueryVO", new Class[] {
	                  ...
	                  cgeneralhid, getEnvironment().getCorpID()
	              });

	      UIDialog uidlg = (UIDialog) Class
	          .forName("nc.ui.scm.so.RushLinkQueryDlg").getConstructor(new Class[] {
	              Container.class, RushLinkQueryVO[].class
	  ...
	  private void onOutReturn() {
	    ...
	    try {
	      ...
	      if(vo.getItemValue(0, IItemKey.CSOURCETYPE)!=null && vo.getItemValue(0, IItemKey.CSOURCETYPE).toString().trim().equals(BillTypeConst.m_allocationOut)){//startsWith("3")){
	        showHintMessage(NCLangRes.getInstance().getStrByID("40080802", "ClientUI-000000")/*直运业务形成的销售出库单不能退回*/);
	        return;
  ...
  /**
   * 从单据表体行，查找出，是成套件的存货，并重构一个新的表体行VO[] 功能： 参数： 返回： 例外： 日期：(2002-04-18 11:29:23)
   * 修改日期，修改人，修改原因，注释标志：
   */
  ...
  protected void setBtnStatusSign(boolean bUpdateButtons) {
    ...
    if (SIGNED == iSignFlag) {
      ...
      if (isCurrentTypeBill()) {
        ...
        getButtonManager().getButton(ICButtonConst.BTN_BILL_DELETE).setEnabled(
            true);
      }
      else {
      ...
      // if (isSetInv(m_voBill, m_iFirstSelectRow) && !isDispensedBill(null))
      if (BillMode.Card == getM_iCurPanel()) {
        getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_DISPENSE)
            .setEnabled(true);
      }
  ...
  protected void setButtonsStatus(int iBillMode) {
    ...
    if (getButtonManager().getButton(ICButtonConst.BTN_ASSIST_COOP_45) != null) {
      if (iBillMode == BillMode.Browse
          ...
          && (null == getCurVO().getHeaderVO().getBsalecooppur() || (null != getCurVO()
              .getHeaderVO().getBsalecooppur()
              && !getCurVO().getHeaderVO().getBsalecooppur().booleanValue()))) {
    ...
    // m_boVerify.setEnabled(false);
    // }
    if (getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_REFER_IN) != null) {
      if (iBillMode == BillMode.Browse)
        getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_REFER_IN)
            .setEnabled(true);
      else
        getButtonManager().getButton(ICButtonConst.BTN_ASSIST_FUNC_REFER_IN)
            .setEnabled(false);
  ...
  /**
   * 创建者：zhx 功能：成套件拆分配件的处理方法。 参数：成套件的存货管理ID，成套件数量（用于计算配件数量） 返回： 例外： 日期：(2001-5-8
   * 19:08:05) 修改日期，修改人，修改原因，注释标志：
   */
    ...
    if (sInvSetID != null) {
      ...
      for (int i = 0; i < rowcount; i++) {
        ...
        UFDouble nchildnum = ((InvVO) alInvvo.get(i))
            .getAttributeValue("childsnum") == null ? new UFDouble(0)
            : new UFDouble(((InvVO) alInvvo.get(i)).getAttributeValue(
                "childsnum").toString());
        ...
        if (nsetnum != null)
          ...
          ntotalnum = nchildnum;
        UFDouble hsl = ((InvVO) alInvvo.get(i)).getAttributeValue("hsl") == null ? null
            : new UFDouble(((InvVO) alInvvo.get(i)).getAttributeValue("hsl")
                .toString());
        ...
        //
        voParts[i]
            .setAttributeValue("csourcetype", headervo.getCbilltypecode());
        voParts[i]
            .setAttributeValue("csourcebillhid", headervo.getPrimaryKey());
        voParts[i].setAttributeValue("csourcebillbid", itemvo.getPrimaryKey());
        voParts[i]
            .setAttributeValue("vsourcebillcode", headervo.getVbillcode());
        ...
        for (int j = 0; j < 20; j++) {
          ...
          voParts[i].setAttributeValue(s + String.valueOf(j + 1), itemvo
              .getAttributeValue(s + String.valueOf(j + 1)));
          voParts[i].setAttributeValue(ss + String.valueOf(j + 1), itemvo
              .getAttributeValue(ss + String.valueOf(j + 1)));
        }
  ...
  public ICBcurrArithUI getCurrArith() {
    ...
    if (clsCurrArith == null) {
      try {
        clsCurrArith = new ICBcurrArithUI(ClientEnvironment.getInstance()
  ...
  public void setListBodyData(GeneralBillItemVO voi[]) {
    ...
    if (voi != null && voi.length > 0) {
      setCurrDigit(BillMode.List, (String) voi[0]
  ...
  /**
   * 创建者：王乃军 功能：在表单设置显示VO 参数： 返回： 例外： 日期：(2001-5-9 9:23:32) 修改日期，修改人，修改原因，注释标志：
   */
  public void setBillVO(GeneralBillVO bvo, boolean bUpdateBotton,
    ...
    if (bvo != null && bvo.getChildrenVO() != null
      ...
      setCurrDigit(BillMode.Card, (String) bvo.getChildrenVO()[0]
          .getAttributeValue("cquotecurrency"));
    }
  ...
  public void setCurrDigit(int ipanelsatus, String cquotecurrency) {
    ...
    try {
      ...
      if (ipanelsatus == BillMode.Card) {
        ...
        BillItem item = getBillListPanel().getBodyBillModel().getItemByKey(
            "nquotemny");
        if (item != null)
          item.setDecimalDigits(iDigit);
      }
  ...
  public boolean isCellEditable(
      boolean value/* BillModel的isCellEditable的返回值 */, int row/* 界面行序号 */,
      String itemkey/* 当前列的itemkey */) {
  ...
  public void getBB3Info(GeneralBillItemVO[] voi, BillModel md) {
    ...
    String[] uikeys = new String[] {
        "naccinvoicenum", "naccountnum1", "nrushnum", IItemKey.naccumoutbacknum
    };
    String[] vofieldkeys = new String[] {
        "nsignnum", "naccountnum1", "nrushnum", IItemKey.naccumoutbacknum
    };
    ...
    if (vofield.size() > 0) {
      try {
        GeneralBillUICtl.fillBodyVOFromBB3(voi, vofield
            .toArray(new String[vofield.size()]), bb3field
            .toArray(new String[bb3field.size()]), type.toIntArray());
...
//      }
  ...
  public void onCoop45Save() {
    ...
    if (null == currBillVO || null == currBillVO.getItemVOs()
      ...
      showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("40080802","UPT40080802-000321"));//请选择一张单据
      return;
    }
    String pk_cusmandoc = (String) currBillVO
        .getHeaderValue(IItemKey.ccustomerid);
    if (null == pk_cusmandoc) {
      showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("40080802","UPT40080802-000322"));//"表头客户为空，不能协同")
      return;
    }
    //修改人：刘家清 修改时间：2008-10-14 下午08:15:21 修改原因：非协同客户也能协同 --对对对！ by liuzy
//    Object[][] objResult = nc.ui.scm.pub.cache.CacheTool.getMultiColValue(
...
//    }
    ...
    if (null != al_splitBills && al_splitBills.size() > 0) {
      ...
      try {
        Object[] obj = nc.ui.pub.pf.PfUtilClient.runBatch(this, "COOPPO",
            ScmConst.m_saleOut, getEnvironment().getLogDate(), vos, vos, null,
            null);
        if (null != obj && obj.length > 0) {
          ...
          if (count > 0 && al_billcodes.size() > 0) {
            String msg =nc.ui.ml.NCLangRes.getInstance().getStrByID("40080802", "UPT40080802-000324", null, new String[]{String.valueOf(count)});// "协同生成" + count + "张采购入库单，单据号如下：";
            for (int i = 0; i < al_billcodes.size(); i++) {
              if (i % 2 == 0)
                msg += "\n";
              msg += al_billcodes.get(i) + ",";
            }
            getCurVO().getHeaderVO().setBsalecooppur(UFBoolean.TRUE);
            if (BillMode.List == getM_iCurPanel()) {
              int selrow = getBillListPanel().getHeadTable().getSelectedRow();
              getBillListPanel().getHeadBillModel().setValueAt(UFBoolean.TRUE,
                  selrow, IItemKey.bsalecooppur);
            }
            else
              getBillCardPanel().getHeadItem(IItemKey.bsalecooppur).setValue(
                  UFBoolean.TRUE);
//            showYesNoMessage(msg);
            nc.ui.pub.beans.MessageDialog.showHintDlg(this, nc.ui.ml.NCLangRes
          .getInstance().getStrByID("SCMCOMMON",
              "UPPSCMCommon-000270")/* @res "提示" */, msg);
            ...
            afterCoop45Save(al_billcodes , currBillVO);
        ...
        showErrorMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("40080802","UPT40080802-000329") + "\n" + e.getMessage());
   ...
   * @throws Exception
   */
  private final void afterCoop45Save(ArrayList<String> al_billcodes , GeneralBillVO currBillVO) throws Exception {
	  
	  // 蓬威石化销售订单协同生成东方希望采购订单处理
	  if("1135".equals(getCorpPrimaryKey())) {
		  if(al_billcodes != null && al_billcodes.size() > 0) {
			  for(String billcode : al_billcodes) {
				  
				  StringBuilder builder = new StringBuilder();
				  builder.append(" select basfil.jobname , genh.dbilldate , genb.cgeneralbid , genb.cinventoryid from bd_jobbasfil basfil  ");
				  builder.append(" left join bd_jobmngfil mngfil on mngfil.pk_jobbasfil = basfil.pk_jobbasfil  ");
				  builder.append(" left join ic_general_b sgenb on sgenb.cprojectid = mngfil.pk_jobmngfil  ");
				  builder.append(" left join ic_general_b genb on genb.csignwastbid = sgenb.cgeneralbid ");
				  builder.append(" inner join ic_general_h genh on genh.cgeneralhid = genb.cgeneralhid  ");
				  ...
				  if(retVector != null && retVector.size() > 0) {
					  ...
					  for(Object vct : retVector) {
						  ...
						  Object jobname = ((Vector)vct).get(0);
						  Object dbilldate = ((Vector)vct).get(1);
						  Object cgeneralbid = ((Vector)vct).get(2);
						  Object cinventoryid = ((Vector)vct).get(3);
						  
						  if(jobname != null && dbilldate != null ) {
...
private ArrayList<GeneralBillVO> filterAndSplitVOs(GeneralBillVO currBillVO) {
	...
	do {
		...
		int i = 0;
		// modified by liuzy 2008-10-31 上午09:05:29 销售订单与销售出库单之间可能会有其它单据，如：发货单、销售发票
		...
		for (GeneralBillItemVO item : items) {
			...
			if (!ScmConst.SO_Order.equals(item.getCfirsttype())
					|| null == sCfirstbillbid
					|| "".equals(sCfirstbillbid)) {
				showHintMessage(nc.ui.ml.NCLangRes.getInstance().getStrByID("40080802","UPT40080802-000325"));//"该单据不符合协同生成采购入库单的条件：来源单据必须是销售订单");
				return null;
			}
