package nc.ui.eh.report.h090601010;
...
import nc.vo.eh.report.h090601010.ReportVO;
...
public class ReportUI extends ToftPanel {
	...
	public ReportBaseClass getReportBase() {
		if (m_report == null)
			try {
				...
				m_report.setTempletID(this.getCorpPrimaryKey(), "H090601010",
						null, null);
			} catch (Exception ex) {
				System.out
						.println("\u57FA\u7C7B:\u672A\u627E\u5230\u62A5\u8868\u6A21\u677F......");
	...
	protected QueryConditionClient createQueryDLG() {
		...
		dlg.setTempletID(this.getCorpPrimaryKey(), "H090601010", null, null);
	...
	public void onQuery() throws Exception {
		...
		if (getQryDlg().getResult() == 1) {
			...
			//包装
			String vPackage = LableAndPackageCode.vPackage;
...
//			.append(" END inprice ")
			...
			.append(" and substr(invcl.invclasscode,1,6)='"+vPackage+"' ")
			...
			UFDouble usedmoney = new UFDouble(0);//耗用资金
			UFDouble usingmoney = new UFDouble(0);	//占用资金
			UFDouble avgkcrl = new UFDouble(0);	//库存容量吨均成本
			UFDouble avgusedmoney = new UFDouble(0);	//耗用吨均支出			
			...
			for (int iam = 0; iam < allAmount.size(); iam++) {
				...
				//耗用资金(耗用量*库存月报表中的期末单价)
				usedmoney = blamount.multiply(kcprice);
				
				//库存容量吨均成本(占用资金/库存容量)
				avgkcrl = usingmoney.div(kcrl);
				
				//耗用吨均支出(耗用资金/产量*(耗用量*规格/1000))
				avgusedmoney = blamount;
				...
				rvo.setKcrl(kcrl);
				rvo.setUsedmoney(usedmoney);
				rvo.setAvgkcrl(avgkcrl);
				rvo.setAvgusedmoney(avgusedmoney);
			...
			if(rvos!=null && rvos.length>0){
                ...
                String[] strValKeys = {"inamont,blamount,thismonthkc,lastmonthkc" +
                		",kcrl,usedmoney,usingmoney,avgkcrl,avgusedmoney,canusedays,neveruse,output,planpurchase"}; 
