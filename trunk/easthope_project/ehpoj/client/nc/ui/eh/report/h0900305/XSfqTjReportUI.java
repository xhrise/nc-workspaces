 ...
 */
package nc.ui.eh.report.h0900305;
 ...
 */
public class XSfqTjReportUI extends ToftPanel {
	...
	public ReportBaseClass getReportBase()
	{
	    if(m_report == null)
	        try
	        {
	            ...
	            m_report.setTempletID(this.getCorpPrimaryKey(), "H0900305", null, null);
	      }
	        catch(Exception ex)
	        {
	...
	public void onQuery() throws Exception{
		   ...
		   if(getQryDlg().getResult() == 1){
			 ...
			 XstjVO[] vos= pubItf.calcXsfqtj(kcVO);
		   	  if (vos!=null&&vos.length>0){
	   	  		...
	   	  		for(int i=0;i<fileds.length;i++){
	   	  			...
	   	  			//if(filed.endsWith("amount")||filed.endsWith("hj")||filed.endsWith("xj")){
	   	  			if(filed.endsWith("amount")||filed.endsWith("hj")||filed.endsWith("xj")||filed.endsWith("mount")){
	   	  			   arrFileds.add(filed);
	   	  			}
	   	  		}
	   	  		//显示合计项
	   	  		String[] strsubValKeys = (String[])arrFileds.toArray(new String[arrFileds.size()]);
		        String[] strgrpValKeys = {"area,custcode"};
...
//		        stctx.setSubtotalName("小计");
		        stctx.setTotalNameColKeys("area");	//设置合计项显示列位置
		        stctx.setSumtotalName("合计");	//设置合计项显示名称
//		        getReportBase().setBodyItemsEditable(strsubValKeys, true);
