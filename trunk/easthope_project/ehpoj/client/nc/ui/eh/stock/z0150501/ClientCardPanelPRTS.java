...
//				m_noteVOs = PfWorkflowBO_Client.queryCheckNote(billcardpanel.getHeadItem(pk_code).getValueObject().toString());
	...
	public String[] getAllDataItemExpress() {
		...
		if (m_billcardpanel.getBillModel()!=null && m_billcardpanel.getBodyItems() != null){
			bodyCount =  m_billcardpanel.getBillModel("eh_stock_contract_b").getBodyItems().length+m_billcardpanel.getBillModel("eh_stock_contract_te").getBodyItems().length+
				m_billcardpanel.getBillModel("eh_stock_contract_ev").getBodyItems().length	;
		}
	...
	public String[] getItemValuesByExpress(String itemExpress) {
		...
		int rowCount  = m_billcardpanel.getBillModel("eh_stock_contract_b").getRowCount()+
                        m_billcardpanel.getBillModel("eh_stock_contract_te").getRowCount()+
                        m_billcardpanel.getBillModel("eh_stock_contract_ev").getRowCount();
		try {
			//表头
			...
			//eh_stock_checkreport为检测报告单表体第一个页签
                ...
                bodyCount = m_billcardpanel.getBillModel("eh_stock_contract_b").getBodyItems().length;
                rowCount = m_billcardpanel.getBillModel("eh_stock_contract_b").getRowCount();
                for (int i = 0; i < bodyCount; i++){
                    BillItem item = m_billcardpanel.getBillModel("eh_stock_contract_b").getBodyItems()[i];
                    if(item==null) return null;
                    ...
                    if (item.getKey().equals(itemExpress)) {
                        ...
                        if(item.getDataType()==4){
                            for (int j = 0; j < rowCount; j++) {
                                if(m_billcardpanel.getBillModel("eh_stock_contract_b").getValueAt(j,  item.getKey())==null){
                                    rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                }else{
                                    if(m_billcardpanel.getBillModel("eh_stock_contract_b").getValueAt(j,  item.getKey()).toString().equals("false")){
                        ...
                        //UIRefPane or UICombox
                            ...
                            for (int j = 0; j < rowCount; j++) {
                                rslt[j] = m_billcardpanel.getBillModel("eh_stock_contract_b").getValueAt(j,  item.getKey())==null?" ":m_billcardpanel.getBillModel("eh_stock_contract_b").getValueAt(j,  item.getKey()).toString();
                 ...
                 bodyCount = m_billcardpanel.getBillModel("eh_stock_contract_te").getBodyItems().length;
                 rowCount = m_billcardpanel.getBillModel("eh_stock_contract_te").getRowCount();
                    for (int i = 0; i < bodyCount; i++){
                        BillItem item = m_billcardpanel.getBillModel("eh_stock_contract_te").getBodyItems()[i];
                        if(item==null) return null;
                        String[] rslt = new String[rowCount];
                        if (item.getKey().equals(itemExpress)) {
                            //UICheckbox
                            if(item.getDataType()==4){
                                for (int j = 0; j < rowCount; j++) {
                                    if(m_billcardpanel.getBillModel("eh_stock_contract_te").getValueAt(j,  item.getKey())==null){
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                    }else{
                                        if(m_billcardpanel.getBillModel("eh_stock_contract_te").getValueAt(j,  item.getKey()).toString().equals("false")){
                                            rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165");
                                        }else{
                                            rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000164")/*@res "是"*/;
                                        }
                            ...
                            //UIRefPane or UICombox
                                ...
                                for (int j = 0; j < rowCount; j++) {
                                    rslt[j] = m_billcardpanel.getBillModel("eh_stock_contract_te").getValueAt(j,  item.getKey())==null?" ":m_billcardpanel.getBillModel("eh_stock_contract_te").getValueAt(j,  item.getKey()).toString();
                                    //rslt[j] = m_billcardpanel.getBodyValueAt(j, item.getKey()) == null ? "" : m_billcardpanel.getBodyValueAt(j, item.getKey()).toString();
                                }
                            }
                            return rslt;
                ...
                bodyCount = m_billcardpanel.getBillModel("eh_stock_contract_ev").getBodyItems().length;
                rowCount = m_billcardpanel.getBillModel("eh_stock_contract_ev").getRowCount();
                for (int i = 0; i < bodyCount; i++){
                    BillItem item = m_billcardpanel.getBillModel("eh_stock_contract_ev").getBodyItems()[i];
                    if(item==null) return null;
                    String[] rslt = new String[rowCount];
                    if (item.getKey().equals(itemExpress)) {
                        //UICheckbox
                        if(item.getDataType()==4){
                            for (int j = 0; j < rowCount; j++) {
                                if(m_billcardpanel.getBillModel("eh_stock_contract_ev").getValueAt(j,  item.getKey())==null){
                                   rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                }else{
                                    if(m_billcardpanel.getBillModel("eh_stock_contract_ev").getValueAt(j,  item.getKey()).toString().equals("false")){
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000165")/*@res "否"*/;
                                    }else{
                                        rslt[j] = nc.ui.ml.NCLangRes.getInstance().getStrByID("uifactory","UPPuifactory-000164")/*@res "是"*/;
                                    }
                        ...
                        //UIRefPane or UICombox
                            ...
                            for (int j = 0; j < rowCount; j++) {
                                rslt[j] = m_billcardpanel.getBillModel("eh_stock_contract_ev").getValueAt(j,  item.getKey())==null?
                                        " ":m_billcardpanel.getBillModel("eh_stock_contract_ev").getValueAt(j,  item.getKey()).toString();
                                //rslt[j] = m_billcardpanel.getBodyValueAt(j, item.getKey()) == null ? "" : m_billcardpanel.getBodyValueAt(j, item.getKey()).toString();
                            }
                        }
                        return rslt;
