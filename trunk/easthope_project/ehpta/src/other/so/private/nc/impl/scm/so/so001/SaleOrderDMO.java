...
//			SaleOrdBalEndDMO balenddmo = new SaleOrdBalEndDMO();
	...
	// 处理查询条件 ：销售订单审核通过 或 未审核但信用证复选框勾选2种情况
	...
	private final String convertSql(String[] whereStrs , String sql) {
		...
		if(whereStrs != null && whereStrs.length > 1) {
			int i = 0;
			for(String sqlWhere : whereStrs) {
				if(i == whereStrs.length - 1)
					newWhere += sqlWhere;
				else
					newWhere += sqlWhere + "(fstatus = 2 or (fstatus = 1 and iscredit = 'Y')) ";
	...
	public Object[] queryAllBillDatas(String sheadsql, String sbodysql) throws BusinessException {
		...
		try {
			...
			// 修改查询审核状态
			// modify by river for 2012-07-22
			CircularlyAccessibleValueObject[] headvos = queryAllHeadData(setSql(sheadsqlnew));
			...
			// 修改查询审核状态
			// modify by river for 2012-07-22
			SaleorderBVO voaItem[] = queryAllBodyDataByWhere(setSql(sbodysql), null);
