...
import nc.ui.hi.hi_303.CardReportViewDlg;
...
public class PsnInfCollectUI extends nc.ui.hr.base.HRToftPanel implements
		...
		BillEditListener, IQueryFieldValueEditor, ILinkMaintain,
		BillEditListener2 {
	...
	private String psnqian;

	private String psn;

	private String psnfen;
	...
	protected ButtonObject[] grpKeyPsnList = { bnPsnGroup, bnKeyPsn, bnBatch,
			bnBatchAddSet, bnUpload, bnCard, bnBook, bnExportPic,
			bnListItemSet, bnSort, bnSetSort, bnQuery, bnFresh, bnPrint };

	// 卡片界面浏览组
	protected ButtonObject[] grpKeyPsnCard = { bnEdit, bnChildEdit, bnSave,
			bnCancel, bnUpload, bnToFirst, bnUpRecord, bnDownRecord, bnToLast,
			bnSubsetMove, bnList };
	...
	protected ButtonObject[] grpCollect = { bnAdd, bnEdit, bnDel, bnBatch,
			bnBatchAddSet, bnIntoDoc, bnUpload, bnCard, bnBook, bnListItemSet,
			bnSort, bnSetSort, bnQuery, bnFresh, bnPrint };// ,

	// 员工信息维护
	protected ButtonObject[] grpSortMaintain = { bnEdit, bnDel, bnBatch,
			bnBatchAddSet, bnEmployeeReference, bnSmUser, bnUpload, bnCard,
			bnBook, bnExportPic, bnListItemSet, bnSort, bnSetSort, bnQuery,
			bnFresh, bnPrint };
	...
	public PsnInfCollectUI() {
		super();
		initialize();
		//this.setDATASOURCE(this.getClientEnvironment().getConfigAccount().getDataSourceName());
	}
	...
	public void adjustQueryDialog() {
		...
		Item item = (Item) getCmbPsncl().getSelectedItem();
		// 调整“是否返聘”可见
		if (item.getValue() == 0 && getModuleName().equalsIgnoreCase("600707")) {
			qts.addFilterByCode(queryDialog, "hi_psndoc_deptchg.isreturn");
		} else if (item.getValue() != 0
				&& getModuleName().equalsIgnoreCase("600707")) {
	...
	private void cmbJobTypItemStateChanged(java.awt.event.ItemEvent e) {
		try {
			// 根据任职类型过滤人员
			Util.getTopFrame(this).setCursor(
					Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
	 ...
	 */
	/* 警告：此方法将重新生成。 */
	private nc.ui.pub.beans.UIPanel getCenterPanelRight() {
	...
	protected HIBillCardPanel getDismissCard() {
		...
		if (this.ivjDismissCard.getHeaderPanel() != null
				&& this.ivjDismissCard.getHeaderPanel("bd_jhk") != null
				&& this.person != null) {
			if (this.ivjDismissCard.getHeaderPanel().getComponent(0) == this.ivjDismissCard
					.getHeaderPanel("bd_jhk").getComponent(0)) {
				getThreespsn();
				this.ivjDismissCard.getHeadItem("def8").setValue(
						this.getPsnqian() == null ? "0.00" : this.getPsnqian());
				this.ivjDismissCard.getHeadItem("def9").setValue(
						this.getPsnfen() == null ? "0.00" : this.getPsnfen());
				this.ivjDismissCard.getHeadItem("def10").setValue(
						this.getPsn() == null ? "0.00" : this.getPsn());
			}
		}
	...
	private PersonEAVO getInputData(String[] tableCodes) throws Exception {
		...
		// System.out.println(eavo.getPsndocVO().getAttributeValue("def1"));
		String OAName = null;
		try {
			OAName = eavo.getPsndocVO().getAttributeValue("def1").toString();
		} catch (Exception ex) {
			// throw new Exception("OA登陆名不能为空！");
			if (eavo.getPsndocVO().getAttributeValue("def2").toString().equals(
					"Y")) {
				eavo.getPsndocVO().setAttributeValue("def2", "N");
				System.out.println("同步标识从 [ Y ] 重置为 [ N ] ");
			}
		}
		IoaqueryList queryList = (IoaqueryList) NCLocator.getInstance().lookup(
				IoaqueryList.class.getName());
		// QueryList queryList = new QueryList();
		if(OAName != null) {
			if (!queryList.checkOANameRep(OAName) && !OAName.equals(firstVal)) {
				// JOptionPane.showMessageDialog(null, "OA登陆名已经存在，请使用其他的登录名进行保存！",
				// "人员信息采集提示", JOptionPane.INFORMATION_MESSAGE);
				throw new Exception("OA登陆名已经存在，请使用其他的登录名进行保存！");
			}
		}
		if (getEditType() == EDIT_MAIN || getEditType() == EDIT_RETURN
	...
	protected HIBillCardPanel getLeaveCard() {
		...
		if (this.ivjLeaveCard.getHeaderPanel() != null
				&& this.ivjLeaveCard.getHeaderPanel("bd_jhk") != null
				&& this.person != null) {
			if (this.ivjLeaveCard.getHeaderPanel().getComponent(0) == this.ivjLeaveCard
					.getHeaderPanel("bd_jhk").getComponent(0)) {
				getThreespsn();
				this.ivjLeaveCard.getHeadItem("def8").setValue(
						this.getPsnqian() == null ? "0.00" : this.getPsnqian());
				this.ivjLeaveCard.getHeadItem("def9").setValue(
						this.getPsnfen() == null ? "0.00" : this.getPsnfen());
				this.ivjLeaveCard.getHeadItem("def10").setValue(
						this.getPsn() == null ? "0.00" : this.getPsn());
			}
		}
		return ivjLeaveCard;
	...
	protected UICheckBox getincludeCancleDept() {
		if (includeCancleDept == null) {
			try {
				...
				includeCancleDept.setPreferredSize(new java.awt.Dimension(100,
						20));
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
	...
	protected UICheckBox getIsSelSet() {
		if (ivjIsSelSet == null) {
			try {
				...
				ivjIsSelSet.setPreferredSize(new java.awt.Dimension(80, 20));
	...
	protected UICheckBox getIsShowSetHistory() {
		if (ivjIsShowSetHistory == null) {
			try {
				...
				ivjIsShowSetHistory.setPreferredSize(new java.awt.Dimension(
						100, 20));
	...
	protected UIPanel getWestPanel() {
		if (WestPanel == null) {
			try {
				...
				WestPanel.setPreferredSize(new java.awt.Dimension(750, 30));
	...
	protected nc.ui.pub.beans.UIPanel getNorthPanel() {
		if (ivjNorthPanel == null) {
			try {
				...
				getNorthPanel().add(getUILabelRecords(), "East");
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	protected HIBillCardPanel getOtherCard() {
		...
		if (this.ivjOtherCard.getHeaderPanel() != null
				&& this.ivjOtherCard.getHeaderPanel("bd_jhk") != null
				&& this.person != null) {
			if (this.ivjOtherCard.getHeaderPanel().getComponent(0) == this.ivjOtherCard
					.getHeaderPanel("bd_jhk").getComponent(0)) {
				getThreespsn();
				this.ivjOtherCard.getHeadItem("def8").setValue(
						this.getPsnqian() == null ? "0.00" : this.getPsnqian());
				this.ivjOtherCard.getHeadItem("def9").setValue(
						this.getPsnfen() == null ? "0.00" : this.getPsnfen());
				this.ivjOtherCard.getHeadItem("def10").setValue(
						this.getPsn() == null ? "0.00" : this.getPsn());
			}
		}
		return ivjOtherCard;
	...
	protected HIBillCardPanel getRetireCard() {
		...
		if (this.ivjRetireCard.getHeaderPanel() != null
				&& this.ivjRetireCard.getHeaderPanel("bd_jhk") != null
				&& this.person != null) {
			if (this.ivjRetireCard.getHeaderPanel().getComponent(0) == this.ivjRetireCard
					.getHeaderPanel("bd_jhk").getComponent(0)) {
				getThreespsn();
				this.ivjRetireCard.getHeadItem("def8").setValue(
						this.getPsnqian() == null ? "0.00" : this.getPsnqian());
				this.ivjRetireCard.getHeadItem("def9").setValue(
						this.getPsnfen() == null ? "0.00" : this.getPsnfen());
				this.ivjRetireCard.getHeadItem("def10").setValue(
						this.getPsn() == null ? "0.00" : this.getPsn());
			}
		}
	...
	protected HIBillCardPanel getWorkCard() {
		...
		// System.out.println(this.ivjWorkCard.getHeaderPanel("bd_jhk"));
		if (this.ivjWorkCard.getHeaderPanel() != null
				&& this.ivjWorkCard.getHeaderPanel("bd_jhk") != null
				&& this.person != null) {
			if (this.ivjWorkCard.getHeaderPanel().getComponent(0) == this.ivjWorkCard
					.getHeaderPanel("bd_jhk").getComponent(0)) {
				getThreespsn();
				this.ivjWorkCard.getHeadItem("def8").setValue(
						this.getPsnqian() == null ? "0.00" : this.getPsnqian());
				this.ivjWorkCard.getHeadItem("def9").setValue(
						this.getPsnfen() == null ? "0.00" : this.getPsnfen());
				this.ivjWorkCard.getHeadItem("def10").setValue(
						this.getPsn() == null ? "0.00" : this.getPsn());
			}
		}
	...
	/**
	 * 获取三个金额数据
	 * add by river for 2011-09-26
	 * @param pk_corp
	 * @param pk_psn
	 */
	public void getThreespsn() {
		try {
			...
			try{
				pk_glbook = String.valueOf(extVO.getPk_glbook());
				
				System.out.println("\n\n pk_glorg  :  " + pk_glbook + "\n\n");
			} catch (Exception e) {
				pk_glbook = "0001AA10000000004VTR";
			}
			
			nc.itf.uap.IUAPQueryBS querybs = (nc.itf.uap.IUAPQueryBS) NCLocator.getInstance().lookup(nc.itf.uap.IUAPQueryBS.class.getName());
			Collection<GlorgbookVO> glbooks = (Collection<GlorgbookVO>)querybs.retrieveAll(GlorgbookVO.class);
			String pk_glorgbook = "";
			for(GlorgbookVO glbook : glbooks) {
				if(glbook.getAttributeValue("pk_glbook").toString().equals("0001AA10000000004VTR")) {
					pk_glorgbook += "'" + glbook.getAttributeValue("pk_glorgbook") + "',";
				}
			}
			
			if(pk_glorgbook.length() > 0)
				pk_glorgbook = pk_glorgbook.substring(0 , pk_glorgbook.length() - 1);
			
			String pk_psndoc = this.person.getPk_psndoc();
			String pk_corp = Global.getCorpPK();
			if(pk_corp.equals("1026")) {
				pk_corp = "1002";
				
				nc.itf.uap.IUAPQueryBS qryBs = (IUAPQueryBS) NCLocator.getInstance().lookup(IUAPQueryBS.class.getName());
				String sql = "select pk_psndoc from bd_psndoc where psnname = '"+this.person.getPsndocVO().getAttributeValue("psnname")+"' and isreferenced = 'Y' and pk_corp = '"+pk_corp+"'";
				pk_psndoc = qryBs.executeQuery(sql, new ColumnProcessor("pk_psndoc")) == null ? "" : qryBs.executeQuery(sql,new ColumnProcessor("pk_psndoc")).toString();
				
			}
			
			

			double qi = getLocalcreditamountsum(basDMO, pk_corp,
					pk_psndoc, pk_glorgbook, date, "122103");
			double benj = getDebitamountsum(basDMO, pk_corp,
					pk_psndoc, pk_glorgbook, date, "122103");
			double benD = getCreditamountsum(basDMO, pk_corp,
					pk_psndoc, pk_glorgbook, date, "122103");
			this.setPsnqian(String.valueOf(benj - benD));
			
			
			
			qi = getLocalcreditamountsum(basDMO, pk_corp,
					pk_psndoc, pk_glorgbook, date, "224104");
			benj = getDebitamountsum(basDMO, pk_corp, 
					pk_psndoc, pk_glorgbook, date, "224104");
			benD = getCreditamountsum(basDMO, pk_corp, 
					pk_psndoc, pk_glorgbook, date, "224104");
			this.setPsnfen(String.valueOf((qi))); //qi - benj + benD
			
			
			
//			qi = getLocalcreditamountsum(basDMO, pk_corp,
//					pk_psndoc, pk_glorgbook, date, "224103");
//			benj = getDebitamountsum(basDMO, pk_corp, 
//					pk_psndoc, pk_glorgbook, date, "224103");
//			benD = getCreditamountsum(basDMO, pk_corp, 
//					pk_psndoc, pk_glorgbook, date, "224103");
//			this.setPsn(String.valueOf((qi - benj + benD)));
	...
	public double getDebitamountsum(IBasDMO basDMO, String pk_corp,
		...
		String sql = "select nvl(sum(sum(gl_detail.debitamount)) , 0) debitamountsum from gl_detail gl_detail,gl_fixtmpfreevalue gl_ftpfreevalue , gl_freevalue free where  gl_detail.pk_accsubj in (select pk_accsubj from bd_accsubj where subjcode='"
				+ subjcode
				+ "' and pk_glorgbook in ("
				+ pk_glorgbook
				+ "))  and gl_detail.yearv='"
				+ date
				+ "' and gl_detail.discardflagv = 'N' and gl_detail.dr = 0 and gl_detail.voucherkindv <> 255  and gl_detail.assid = gl_ftpfreevalue.assid "
				+ " and gl_ftpfreevalue.assid = free.freevalueid and gl_ftpfreevalue.def4 in (select pk_psndoc from bd_psndoc where pk_psnbasdoc in (select pk_psnbasdoc from bd_psndoc where pk_psndoc = '"+pk_psn+"'))  and free.valuecode = gl_ftpfreevalue.code4 "
		...
		try {
			nc.itf.uap.IUAPQueryBS querybs = (nc.itf.uap.IUAPQueryBS) NCLocator.getInstance().lookup(nc.itf.uap.IUAPQueryBS.class.getName());
			String num = String.valueOf(querybs.executeQuery(sql, new ColumnProcessor()));
//			String num = basDMO.getStr(sql, this.getClientEnvironment().getConfigAccount().getDataSourceName());
			debitamountsum = Double.parseDouble(num == null || "null".equals(num) ? "0" : num);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public double getCreditamountsum(IBasDMO basDMO, String pk_corp,
			String pk_psn, String pk_glorgbook, String date, String subjcode) {
		double creditamountsum = 0;
		String sql = "select nvl(sum(sum(gl_detail.creditamount)) , 0) creditamountsum from gl_detail gl_detail,gl_fixtmpfreevalue gl_ftpfreevalue , gl_freevalue free where gl_detail.pk_accsubj in (select pk_accsubj from bd_accsubj where subjcode='"
				+ subjcode
				+ "' and pk_glorgbook in ("
				+ pk_glorgbook
				+ "))  and gl_detail.yearv='"
				+ date
				+ "'  and gl_detail.discardflagv = 'N' and gl_detail.dr = 0 and gl_detail.voucherkindv <> 255  and gl_detail.assid = gl_ftpfreevalue.assid and gl_ftpfreevalue.def4  in (select pk_psndoc from bd_psndoc where pk_psnbasdoc in (select pk_psnbasdoc from bd_psndoc where pk_psndoc = '"+pk_psn+"')) " + 
				" and gl_ftpfreevalue.assid = free.freevalueid and free.valuecode = gl_ftpfreevalue.code4 group by  gl_detail.pk_accsubj,gl_detail.pk_glorgbook,gl_detail.yearv,gl_detail.assid order by  gl_detail.pk_accsubj,gl_detail.pk_glorgbook,gl_detail.yearv,gl_detail.assid ";
		try {
			nc.itf.uap.IUAPQueryBS querybs = (nc.itf.uap.IUAPQueryBS) NCLocator.getInstance().lookup(nc.itf.uap.IUAPQueryBS.class.getName());
			String num = String.valueOf(querybs.executeQuery(sql, new ColumnProcessor()));
//			String num = basDMO.getStr(sql, this.getClientEnvironment().getConfigAccount().getDataSourceName());
			creditamountsum = Double.parseDouble(num == null || "null".equals(num) ? "0" : num);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public double getLocalcreditamountsum(IBasDMO basDMO, String pk_corp,
			String pk_psn, String pk_glorgbook, String date, String subjcode) {
		double localcreditamountsum = 0;
		
		StringBuffer sb = new StringBuffer();
		sb.append("select nvl(sum(localcreditamount) , 0) as localcreditamount from gl_balance blc left join gl_freevalue free on free.freevalueid = blc.assid ");
		sb.append("left join gl_fixtmpfreevalue fixtmp on fixtmp.assid = free.freevalueid ");
		sb.append("where blc.pk_corp in (select pk_corp from bd_psndoc where pk_psnbasdoc in (select pk_psnbasdoc from bd_psndoc where pk_psndoc = '"+pk_psn+"')) and blc.year = '"+date+"' ");
		sb.append("and blc.pk_accsubj in (select pk_accsubj from bd_accsubj where subjcode='"+subjcode+"' and pk_glorgbook in ( "+pk_glorgbook+" )) ");
		sb.append("and (fixtmp.def4  in (select pk_psndoc from bd_psndoc where pk_psnbasdoc in (select pk_psnbasdoc from bd_psndoc where pk_psndoc = '"+pk_psn+"')) ) and free.valuecode = fixtmp.code4 ");
...
//			String num = basDMO.getStr(sql, this.getClientEnvironment().getConfigAccount().getDataSourceName());
			if(num.equals(""))
				num = null;
			localcreditamountsum = Double.parseDouble(num == null || "null".equals(num) ? "0.0" : num);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return localcreditamountsum;
	}
	...
	public void initConnections() throws java.lang.Exception {
		...
		// getCard().getHeadItem("def1").getComponent().addValueChangedListener
			...
			if (getModuleName().equalsIgnoreCase("600704")) {
				...
				if (intodocdirect == 1) {
					ButtonObject[] grpCollectIndoc = { bnAdd, bnEdit, bnDel,
							bnBatch, bnBatchAddSet, bnUpload, bnCard, bnBook,
							bnIndocApp, bnListItemSet, bnSort, bnSetSort,
	...
	protected Pair[] convertToPairs(GeneralVO[] itemVOs) {
		...
		if (itemVOs != null && itemVOs.length > 0) {
			Pair unitpair = new Pair("unitname",
					nc.ui.ml.NCLangRes.getInstance().getStrByID("600704",
							"upt600704-000047")/* @res "公司名称" */, 0, "",
					"bd_corp", true);
			...
			if (getModuleName().equals("600710")) {
				...
				if (getModuleName().equals("600707")) {
					Pair jobtypepair = new Pair("jobtypename",
							nc.ui.ml.NCLangRes.getInstance().getStrByID(
									"600704", "UPP600704-000065")/*
																	 * @res
																	 * "任职类型"
																	 */, 0, "", "hi_psndoc_deptchg", true);
	...
	protected void onCard() throws java.lang.Exception {
		...
		if (pk_psndoc != null) {
			...
			CardReportViewDlg dlg = new CardReportViewDlg(this);
			dlg.resetDlg(getClientEnvironment().getUser().getPrimaryKey(),
					corpPK, pk_psndoc, pk_psndoc_sub, jobtype);
			dlg.showModal();
		}
	}
	...
	private void save() throws java.lang.Exception {
		...
		if (getEditType() == EDIT_MAIN || getEditType() == EDIT_RETURN) {
			UFBoolean check = null;
			try{
				check = (UFBoolean)eavo.getPsndocVO().getAttributeValue("def7");
			} catch(Exception ex) {
				check = new UFBoolean("N");
			}
			
			eavo.getPsndocVO().setAttributeValue("def7", "N");
			saveMain(eavo);

			// ---------- 添加同步OA方法 add by river ------------ //
			
			String datasource = this.getClientEnvironment().getConfigAccount().getDataSourceName();
			if(datasource == null)
				datasource = "nc56true";
			...
			if (!eavo.getParentVO().getAttributeValue("approveflagName")
				...
				if (!eavo.getParentVO().getAttributeValue("approveflagName")
					...
					try {
						if (eavo.getPsndocVO().getAttributeValue("def2").toString() == "Y"
							&& !(eavo.getPsndocVO().getAttributeValue("def1") == null || eavo
...
//							}
						...
						if (eavo.getPsndocVO().getAttributeValue("def3") == null
								|| eavo.getPsndocVO().getAttributeValue("def3")
										.equals("")) {
							String msg = mbSyn.create(datasource, eavo
									.getParentVO().getAttributeValue("pk_psndoc")
									.toString(), eavo.getPsndocVO()
									.getAttributeValue("def2").toString());
							System.out.println(msg);
						}
	
						if (eavo.getPsndocVO().getAttributeValue("def2").toString() == "N"
								&& !(eavo.getPsndocVO().getAttributeValue("def1") == null || eavo
										.getPsndocVO().getAttributeValue("def1")
										.toString().trim().equals("")))
							mbSyn.update1(eavo.getPsndocVO().getAttributeValue(
									"def1").toString().trim(), eavo.getPsndocVO()
									.getAttributeValue("pk_psndoc").toString(),
									eavo.getPsndocVO().getAttributeValue("pk_corp")
											.toString(), Long.valueOf(eavo
											.getPsndocVO()
											.getAttributeValue("def3").toString()),
											datasource, false);
						else {
							if (!(eavo.getPsndocVO().getAttributeValue("def3") == null)) {
								
								boolean bool = true;
								if(eavo.getPsndocVO().getAttributeValue("def2").toString().equals("N") && eavo.getPsndocVO().getAttributeValue("def1") == null) {
									bool = false;
								}
								
								mbSyn.update(eavo.getPsndocVO().getAttributeValue(
										"pk_psndoc").toString(), eavo.getPsndocVO()
										.getAttributeValue("pk_corp").toString(),
										Long.valueOf(eavo.getPsndocVO()
												.getAttributeValue("def3")
												.toString()), datasource,
												bool , check.booleanValue());
	...
	public String getDatasourse() {
		...
		try {
			...
			if (!bl) {
				// String path =
				// QueryList.class.getClass().getResource("/").getPath();
	...
	protected void setBillItemEditable() {
		...
		UICheckBox checkBox = (UICheckBox) getBillItem("bd_psndoc.clerkflag")
				.getComponent();
		// 设置职员编码控件不可编辑
	...
	protected PersonEAVO loadPsnMainInfo(int selectRow, String tablecode) {
		...
		if (getCard().getHeadItem("def1") == null) {
			firstVal = null;
		} else {
			firstVal = getCard().getHeadItem("def1").getValue();
		}
		return person;
	}
	...
	protected UICheckBox getincludeChildDeptPsn() {
		if (includeChildDeptPsn == null) {
			try {
				...
				includeChildDeptPsn.setPreferredSize(new java.awt.Dimension(
						100, 20));
				// 默认包含下级人员
	...
	public void afterEdit(BillEditEvent e) {
		...
		if (key.equals("def1")) {
			IoaqueryList queryList = (IoaqueryList) NCLocator.getInstance()
					.lookup(IoaqueryList.class.getName());
			// QueryList queryList = new QueryList();
			boolean tof;
			try {
				
				tof = queryList.checkOANameRep(e.getValue().toString());

				if (!tof && !firstVal.equals(e.getValue().toString())) {
					// JOptionPane.showMessageDialog(null,
					// "OA登陆名已经存在，请使用其他的登录名进行保存！", "人员信息采集提示",
					// JOptionPane.INFORMATION_MESSAGE);
					this.showWarningMessage("OA登陆名已经存在，请使用其他的登录名进行保存！");
					getCard().getHeadItem(key).setValue(beforValue);
				}
			} catch (Exception e1) {
	...
	public boolean beforeEdit(BillEditEvent arg0) {
		...
		if ("def1".equals(key)) {
			beforValue = ((UIRefPaneTextField) arg0.getSource()).getText();
		}
		return false;
	}
	...
	public String getPsnfen() {
		return psnfen;
	}

	public void setPsnfen(String psnfen) {
		this.psnfen = psnfen;
	}

	public String getPsnqian() {
		return psnqian;
	}

	public void setPsnqian(String psnqian) {
		this.psnqian = psnqian;
	}
}
