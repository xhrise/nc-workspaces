...
import nc.ui.hbbb.pub.HBBBRepUtil;
...
public class ReportCommitAction extends MultiFrameAction {
	 ...
	 * @throws WebException
	 */
	public ActionForward doCommitCancel(ActionForm actionForm) throws WebException {
		try{
			//从界面上取得用户选择了哪些aloneid、repid的数据取消上报
			Vector<ReportCommitVO> vCommit=new Vector<ReportCommitVO>();
			...
			String unit_code = "";
			String whereSql = "";
			for(int i = 0 ; i < vCommit.size() ; i++){
				reportcodes += "'" + queryList.getReportcode(vCommit.get(i).getRepId()) + "',";
//				repIds += "'" + vCommit.get(i).getRepId() + "',";
//				aloneIds += "'" + vCommit.get(i).getAloneId() + "',";
				
				whereSql += "(rep_id = '" + vCommit.get(i).getRepId() + "' and alone_id = '" + vCommit.get(i).getAloneId() + "') or ";
				
				boolean check = true;
				boolean checkcode = true;
				String[] retUnit = queryList.getUnitId(commitVOs[i].getCommitUnitPK());
				if(unit_name != null){
					String[] units = unit_name.split(",");
					String[] unitcodes = unit_code.split(",");
					for(String unit : units){
						String newUnit = "'"+retUnit[0]+"'";
						if(unit.equals(newUnit))
							check = false;
					}
					
					for(String unit : unitcodes){
						String newUnit = "'"+retUnit[1]+"'";
						if(unit.equals(newUnit))
							checkcode = false;
					}
				}
			...
			reportcodes = reportcodes.substring(0 , reportcodes.length() - 1);
			unit_name = unit_name.substring(0 , unit_name.length() - 1);
			unit_code = unit_code.substring(0 , unit_code.length() - 1);
			String loginDate = null;
			
			try{
				for(java.util.Iterator itr = hashPubData.keySet().iterator(); itr.hasNext();){ 
					String key = (String) itr.next(); 
					MeasurePubDataVO pubdate =  hashPubData.get(key); 
					loginDate = pubdate.getInputDate().substring(0 , 7); 
				} 
			}catch(Exception ex){
				String retMsg = "获取到的报表时间出错，请检查！";
				AppDebug.debug(retMsg);
				return new CloseForward(CloseForward.CLOSE);
			}
			String retMsg = this.CancleCommitClient(unit_name, reportcodes , unit_code , loginDate);
			if(retMsg != null){
				try {
					
						String content = retMsg;
						String title = "取消上报请求";
						...
						for (String userId : userList) {
							if(retMsg == "请求失败！" || retMsg.equals("请求失败！"))
								content = retMsg + " 本地将不执行报表的取消上报！";
						...
						queryList.delRepCommit(whereSql);
				} catch (Exception ex) {
					AppDebug.debug(ex);
				}
			}
...
//            fwd.setRedirect(true);
	...
	public String CancleCommitClient(String unit_name , String reportcodes , String unit_code , String loginDate) throws ServiceException,
	RemoteException, Exception{
		...
		String url = "http://" + ip.item(0).getTextContent() + "/axis/CancleCommitService.jws?wsdl";
		System.out.println("WEBSERVICE的地址：" + url);
		AppDebug.debug("WEBSERVICE的地址：" + url);
		try{
...
//					JAFDataHandlerDeserializerFactory.class);
			call.setOperationName(new QName(url, "CancleCommit"));
...
//			call.addParameter("handler", qn, ParameterMode.IN);
			...
			call.addParameter("reportcodes", XMLType.XSD_STRING, ParameterMode.IN);
			call.addParameter("unit_code", XMLType.XSD_STRING, ParameterMode.IN);
			call.addParameter("loginDate", XMLType.XSD_STRING, ParameterMode.IN);
			// 设置返回值类型，下面2种方法都可以
			call.setReturnClass(String.class);
			// call.setReturnType(XMLType.XSD_STRING);

			String result = (String) call.invoke(new Object[] {unit_name , reportcodes , unit_code , loginDate});
			System.out.println(result);
			return result;
		} catch (Exception e) {
			e.printStackTrace();
			AppDebug.debug("请求取消上报操作时出现未知错误！");
			return "请求取消上报操作时出现未知错误！";
		}
	}
	 ...
	 * @throws Exception
	 */
	public ReportCommitVO[] adjustCommit(ReportCommitVO[] commits,String strTimeProp,TaskVO taskVO,Vector<ReportCommitVO> vExportCommit) throws Exception{
		if (commits==null || commits.length<=0)
			return commits;
		
		KeyGroupVO keyGroup=taskVO==null?null:IUFOUICacheManager.getSingleton().getKeyGroupCache().getByPK(taskVO.getKeyGroupId());
		...
		//V5.3  vRetCommit存储上报单位和上报用户的信息,2008-03-13 liulp
		...
		for (int i=0;i<vAloneID.size();i++){
			...
			for (int j=0;j<vRepID.size();j++){
				...
				if (taskVO!=null){
					...
					//V5.3,2008-03-13 liulp, 去掉请求取消标记时,重新设置上报单位和上报用户的信息
					if (oneAloneIDCommitVOs!=null && j<oneAloneIDCommitVOs.length && oneAloneIDCommitVOs[j]!=null && oneAloneIDCommitVOs[j].getCommitFlag()>ReportCommitVO.NOTCOMMITED){
						...
						setCommitUnitUserInfo(oneAloneIDCommitVOs[j]);
						vRetCommit.add(oneAloneIDCommitVOs[j]);
						continue;
					}
					
					//改上标记为上报，判断是否是补报
					commit.setCommitFlag(isAfterCommited(taskVO, keyGroup, pubDatas[i])?ReportCommitVO.AFTERCOMMITED:ReportCommitVO.COMMITED);
					setCommitUnitUserInfo(commit);
					...
					if (oneAloneIDCommitVOs!=null && j<oneAloneIDCommitVOs.length && oneAloneIDCommitVOs[j]!=null){
						//"请求取消"不改变表上报单位和上报用户setCommitUnitUserInfo(oneAloneIDCommitVOs[j])
						vRetCommit.add(oneAloneIDCommitVOs[j]);
	...
	/**
	 * V5.3  存储上报单位和上报人信息,2008-03-13 liulp
	 * @param commitVO
	 */
	private void setCommitUnitUserInfo(ReportCommitVO commitVO) {
		if(commitVO == null){
			return;
		}
		String strCommitUnitPK = getCurUserInfo().getUnitId();
		String strCommitUserPK = getCurUserInfo().getID();
		commitVO.setCommitUnitPK(strCommitUnitPK);
		commitVO.setCommitUserPK(strCommitUserPK);	
	}
	
	public static boolean isAfterCommited(TaskVO task,KeyGroupVO groupvo,MeasurePubDataVO pubData){
		if (task.getEndDatePoint()==null || task.getEndDatePoint().trim().length()<=0)
			return false;
		
		String strTimeProp=groupvo.getTimeProp();
		UFODate ufoDate=null;
		if (strTimeProp!=null && strTimeProp.equals(UFODate.NONE_PERIOD)==false){
			ufoDate = new UFODate(pubData.getInputDate());
			ufoDate = ufoDate.getEndDay(strTimeProp);
		}else if (groupvo.getAccKey()!=null){
			KeyVO key=groupvo.getAccKey();
			String[] strNatDates=AccPeriodSchemeUtil.getInstance().getNatDateByAccPeriod(task.getAccPeriodScheme(), key.getKeywordPK(), pubData.getInputDate());
			if (strNatDates!=null)
				ufoDate=new UFODate(strNatDates[1]);
		}
		
		if (ufoDate!=null){
			int nEndDatePoint = Integer.parseInt(task.getEndDatePoint());
			String strEndTimePoint = task.getEndTimePoint();
			return ReportCommitAction.isAfterTime(ufoDate, nEndDatePoint, strEndTimePoint,null);
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static boolean isAfterTime(UFODate ufoDate,int nEndDatePoint,String strEndTimePoint,String strDateTimeDst){
		if(ufoDate == null){
			return false;
		}
	...
	public static boolean isAfterCommited(String strDateSrc,int nEndDatePoint,String strEndTimePoint){
		...
		return isAfterTime(ufoDate,nEndDatePoint,strEndTimePoint,null);
	}
	/**
	 * 从界面上得到用户选择的上报记录
	 * @param taskVO
	...
	private void loadCheckInputResults(String strTaskID,List<String> vAloneID1,Map<String,List<String>> hashPassedRepID,Map<String,List<String>> hashInputRepID,Map<String,Boolean> hashTaskCheck,Map<String,MeasurePubDataVO> hashPubData,boolean bMustCheckPass) throws Exception{
		...
		for (int i=0;i<pubDatas.length;i++){
			...
			CheckResultVO[] chres = CheckResultBO_Client.loadTaskCheckResults(false,strTaskID,CheckResultVO.CHECK_ALLSTATE, new String[]{strAloneID},null);
	 ...
	 * @throws Exception
	 */
	public ActionForward loadCommitFromUI(Vector<ReportCommitVO> vRetCommitVO,Vector<String> vBalErrMsg,Vector<String> vCommitErrMsg,Hashtable<String,MeasurePubDataVO> hashPubData,boolean bMustCheckPass,boolean bCommit) throws Exception{
		...
		for (int i = 0; i < vecCommitVOs.size(); i++) {
			...
			if (repVOs[0] != null){
				...
				if (!HBBBSysParaUtil.isHBBBAndSWVer(pubData.getVer())){
					...
					strTaskRepPKs = HBBBRepUtil.getNonVarRepIDs(taskId);
				}
	...
	private String getKeywordStrByAloneID(String strAloneID){
		...
		for (int i=0;i<keys.length;i++){
			...
			if (i<keys.length-1)
				buf.append(StringResource.getStringResource("miufo1001022"));
		}
	 ...
	 * @param hashPubData
	 * @return
	 * @i18n miufo00887=参数控制只有本单位用户才可以请求取消上报
	 */
	private ReleaseInfoValue sendCancelCommit(ReportCommitVO[] commitVOs, String taskId,Hashtable<String,MeasurePubDataVO> hashPubData) {
	    try {
            ...
            boolean bNeedSelfAsk="true".equalsIgnoreCase(SysPropMng.getSysProp(ISysProp.NEED_SELF_ASKCANCELCOMMIT).getValue());
	        ...
	        for (int i = 0; i < commitVOs.length; i++) {           
	            ...
	            if (bHasUnitKey && bNeedSelfAsk){
	            	if (!userInfo.getUnitId().equals(pubData.getUnitPK()))
	            		throw new CommonException(StringResource.getStringResource("miufo00887"));
	            }
