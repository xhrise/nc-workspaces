...
public class PercentLayout implements LayoutManager2 {
  ...
  private static class NumberConstraint extends Constraint {
    public NumberConstraint(double d) {
      this(new Double(d));
    }
    public NumberConstraint(Double d) {
      super(d);
  ...
  private static class PercentConstraint extends Constraint {
    public PercentConstraint(double d) {
      super(new Double(d));
    }
  ...
  public void addLayoutComponent(Component component, Object constraints) {
    if (constraints instanceof Constraint) {
      ...
      addLayoutComponent(
        component,
        new NumberConstraint(((Number)constraints).doubleValue()));
    } else if ("*".equals(constraints)) {
      addLayoutComponent(component, REMAINING_SPACE);
    } else if ("".equals(constraints)) {
      addLayoutComponent(component, PREFERRED_SIZE);
    } else if (constraints instanceof String) {
      String s = (String)constraints;
      if (s.endsWith("%")) {
        double value =
          Double.valueOf(s.substring(0, s.length() - 1)).doubleValue() / 100;
        if (value > 1 || value < 0)
          throw new IllegalArgumentException("percent value must be >= 0 and <= 100");
        addLayoutComponent(component, new PercentConstraint(value));
      } else {
        addLayoutComponent(component, new NumberConstraint(Double.valueOf(s)));
      }
  ...
  public void layoutContainer(Container parent) {
    ...
    for (int i = 0, c = components.length; i < c; i++) {
      ...
      } else if (constraint instanceof PercentConstraint) {
        sizes[i] =
          (int) (totalSize * ((PercentConstraint)constraint).doubleValue());
      } else if (constraint == REMAINING_SPACE) {
