...
public class ChoosePasteDlg extends UfoDialog implements ActionListener{
	private static final long serialVersionUID = -7974489100433201147L;
	...
	/** 返回的对象 */
	private AbsPasteExecutor choosePaste = null;
	/** 表格组件 */
	...
	private JRadioButton getAllRadio() {
		if (allRadio == null) {
			try {
				...
				setChoosePaste(new PasteAll(mainBoard, false));
		...
		if (source instanceof JButton) {
			...
			if (btSource == btOk) { // 确定按钮的处理
				...
				if (allRadio.isSelected()) {
					setChoosePaste(new PasteAll(mainBoard, b_isSelected));
				} else if (valueRadio.isSelected()) {
					setChoosePaste(new PasteContent(mainBoard, b_isSelected));
				} else if (formatRadio.isSelected()) {
					setChoosePaste(new PasteFormat(mainBoard, b_isSelected));
				} else if (postilRadio.isSelected()) {
					setChoosePaste(new PastePostil(mainBoard, b_isSelected));
				} else if (noBorderRadio.isSelected()) {
					setChoosePaste(new PasteNoBorder(mainBoard, b_isSelected));
				} else if (columnWidthRadio.isSelected()) {
					setChoosePaste(new PasteColumnWidth(mainBoard, b_isSelected));
				} else if (valueNumFormatRadio.isSelected()) {
					setChoosePaste(new PasteValueNumFormat(mainBoard, b_isSelected));
				}
				this.close();
				return;
	...
	public AbsPasteExecutor getChoosePaste() {
		return choosePaste;
	}

	public void setChoosePaste(AbsPasteExecutor choosePaste) {
