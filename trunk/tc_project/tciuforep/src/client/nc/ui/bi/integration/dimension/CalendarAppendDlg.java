 ...
 */
public class CalendarAppendDlg extends WebDialog {
	/**
	 * 
	 */
	...
	/**
	 * @i18n mbidim00103=成员编码或名称已存在时，是否覆盖
	 * @i18n miufo00380=成员编码或名称已存在时，是否覆盖
	 */
//	private final static String CB_TXT_REPLACE = StringResource.getStringResource("mbidim00103");//TODO
	private final static String CB_TXT_REPLACE = StringResource.getStringResource(StringResource.getStringResource("miufo00380"));
	private final static String CHECKBOX_ID = "isReplace";
	...
	private WebCheckBox cb_replace = null;
	...
	protected void setData() throws WebException {
		...
		cb_replace.setLabel(CB_TXT_REPLACE);
		cb_replace.setValue(false);
	...
	protected WebPanel getFieldPane() {
		if (fieldPane == null) {
			...
			fieldPane.setLayout(new WebGridLayout(4, 4));
...
//			fieldPane.add(new WebLabel(StringResource.getStringResource("mbidim00069")), new Area(4, 1, 1, 1));
			//fieldPane.add(new WebLabel(""), new Area(2, 1, 1, 1));
			cb_replace = new WebCheckBox(CHECKBOX_ID);
			cb_replace.setID(CHECKBOX_ID);
			fieldPane.add(cb_replace, new Area(4, 1, 3, 1));
	...
	protected WebPanel getBtnPane() {
		if (btnPane == null) {
			...
			ActionForward fwd = new ActionForward(CalendarDesignAction.class.getName(), CalendarDesignAction.METHOD_SUBMIT_APPEND);
