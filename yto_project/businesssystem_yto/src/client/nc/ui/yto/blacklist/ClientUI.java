	...
	public void afterEdit(BillEditEvent event) {
		...
		try {
			...
			if (pk_psnbasdoc != null) {
				...
				String str = blkQuery.getStr("select 1 from hi_psndoc_bad where id = '"+psnbas.getId()+"'", "yt_test");
				if("1".equals(str)) {
					JOptionPane.showMessageDialog(null, "该人员已经存在于黑名单中，无需重复添加！" , "提示" , JOptionPane.OK_OPTION);
...
//				panel.getBodyItem("permanentres").setValue(psnbas.getBasgroupdef18());
				String per = psnbas.getPermanreside();
				String docname = blkQuery.getStr("select docname from bd_defdoc where pk_defdoc = '"+per+"' ", "yt_test");
