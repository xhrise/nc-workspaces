	...
	public void execute() throws Exception {
		...
		try {
			...
			if (curSelectedVO.getIbillstate() == IBillStatus.CHECKPASS) {// 如果当前单据是审批通过状态 
				...
				//modify by fanghfa 20121114 通过设置参数，来判定审批通过后是否需要发送消息通知
				String isSendMessage = PubDelegator.getIParValue().getParaString("0001", "HI_SENDMESSAGE"); //取得“人事单据审批通过后是否发送通知”的参数值
				if(isSendMessage == null || UFBoolean.valueOf(isSendMessage.toString().trim()).booleanValue()){
		...
		try {
			...
			for (int i = 0; i < pk_psndocs.length; i++) {
				...
				for (RegularapplBVO j : bodyVO) {
					...
					//2011-8-9 旧数据参数名修正
					else if (j.getPk_flddict().equalsIgnoreCase("oldpk_deptdoc")) {
						oldPsnVO.setAttributeValue("pk_deptdoc", j.getVvalue());
					}
					else if (j.getPk_flddict().equalsIgnoreCase("newpk_deptdoc")) {
						newPsnVO.setAttributeValue("pk_deptdoc", j.getVvalue());
					}
				}
