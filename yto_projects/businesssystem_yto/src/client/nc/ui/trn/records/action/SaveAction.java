...
//	 是否同步工作履历
	...
	private List<nc.vo.yto.business.PsndocVO> PsndocUpdate = new ArrayList<nc.vo.yto.business.PsndocVO>();
	private List<nc.vo.yto.business.PsndocVO> PsndocUpdate2 = new ArrayList<nc.vo.yto.business.PsndocVO>();
...
// 人员调配或离职保存时同步至中间表 add by river for 2011-09-15
		PsndocVO psndocVO =(PsndocVO) hrAggVO.getParentVO();
		String pk_psndoc = psndocVO.getPk_psndoc().toString().trim();
		...
		nc.vo.yto.business.PsndocVO psndocVO2 = ((nc.vo.yto.business.PsndocVO[])msg.getGeneralVOs(nc.vo.yto.business.PsndocVO.class, " pk_psndoc = '"+pk_psndoc+"'"))[0];
		
		String retStr = filepost.postFile(Uriread
				.uriPath(), gener.generateXml4(psndocVO2, "RequestPsndoc",
				"psn", "update" , opmsg));
				...
				public void run() {
					...
					try {
						if(true) {
							...
							for(nc.vo.yto.business.PsndocVO psn : PsndocUpdate) {
							...
							PsndocUpdate2 = new ArrayList<nc.vo.yto.business.PsndocVO>();
...
//		 本记录的外键
		...
		FlddictVO[] flddictTabVOs = null;
		try {
			flddictVOs = setdictServer.queryAllSonForCode(
					getDataMdl().getPk_corp(), vo.getTableName(), null, true,
					true);
			if(!strTabCode.equals(vo.getTableName())){
				flddictTabVOs = setdictServer.queryAllSonForCode(
						getDataMdl().getPk_corp(), strTabCode, null, true,
						true);
			}
		} catch (Exception e) {
		...
		for(FlddictVO o: flddictVOs){
			if (!o.getFldcode().equals(psnDataVO.getPkname())
				...
				if (getCurBillModel().getItemByKey(o.getFldcode())!=null) {
					//fengwei 2010-11-18 增加离职记录时，如果“是否在岗”字段为空，则默认为false  NCdp202746054
					if("poststat".equals(o.getFldcode()) && CommonValue.HI_PSNDOC_DIMISSION.equals(strTabCode) 
							&& vo.getAttributeValue(o.getFldcode()) == null){
						valuevec.add(UFBoolean.FALSE);
					}else {
					...
					valuevec.add("N");
				}else if("bendflag".equalsIgnoreCase(o.getFldcode())){
					if(vo.getAttributeValue(o.getFldcode())!=null){
						valuevec.add(vo.getAttributeValue(o.getFldcode()));
					}else{
						valuevec.add(null);
					}
		...
		//增加当前编辑标签页中的自定义字段
		if(flddictTabVOs!=null){
			for(FlddictVO o: flddictTabVOs){
					if(o.getFldcode().startsWith("groupdef")
							&&getCurBillModel().getItemByKey(o.getFldcode())!=null){
						//fengwei 2010-12-6 只有前面没有的自定义项才加入　
						if(namevec.contains(o.getFldcode())){
							continue;
						}
						namevec.add(o.getFldcode());
						valuevec.add(vo.getAttributeValue(o.getFldcode()));
		...
		if (CommonValue.HI_PSNDOC_PART.equals(strTabCode) && !namevec.contains("bendflag")) {
...
//		BDFormuleVO[] tongbuFields = getBDFormulas(strTabCode);
	...
	public boolean validate() throws ValidationException {
		...
		saveSubData = vos[0];
		//对于getBodyValueChangeVOs函数，新增的记录，字段为逻辑型的，会返回为空，而不是，UFBoolean（false）。因此需要设置一下。
		if("hi_psndoc_deptchg".equalsIgnoreCase(strTabCode)||"hi_psndoc_part".equalsIgnoreCase(strTabCode)){
			if(saveSubData.getAttributeValue("bendflag")==null) saveSubData.setAttributeValue("bendflag", UFBoolean.valueOf(false));
		}
...
//		isInster = false;
	...
	private void updateChild(HRSubVO vo, int i,HRSubVO voname, boolean isInPhase) throws BusinessException {
		...
		TRNDelegator.getStapplybH().updatePsnData((nc.vo.hi.hi_401.PsnDataVO) vo,i, (nc.vo.hi.hi_401.PsnDataVO) voname, isInPhase, isBefore);
	...
	private void validateDeptchg() throws ValidationException {
		...
		//1代表表体
		int index =  1;
		BillItem item = this.getMainPnl().getRefItemByKey(index, strTabCode, "pk_om_duty");
		BillData data = new BillData();
		data.setHeadItems(new BillItem[]{item});
		if (!data.execValidateFormulas())
			throw new ValidationException("页面数据校验错误,请重新输入");		
	...
	private void validatePart() throws ValidationException {
		...
		if (pk_postdoc != null && isJobAbortedls(pk_postdoc)) {
			throw new ValidationException(
		...
		if (!dept.booleanValue()
				&& ((getDeptCanceledls().contains(pk_deptdoc))
						||( pk_postdoc != null && isJobAbortedls(pk_postdoc)))) {
	...
	/**
	 * @author lianglj 
	 * add by 2010-09-10 根据picc需求增加，已经撤销的数据过多时全部查询会造成内存溢出
	 * @param pk_postdoc
	 * @return
	 */
	public Boolean isJobAbortedls(String pk_postdoc) {
		Boolean isJobAbort = false;
			try {
				//shien 2012-7-26
				JobVO[] vos = (JobVO[])PubDelegator.getIPersistenceRetrieve().retrieveByClause(null, 
						JobVO.class, " isabort='Y' and pk_om_job='" + pk_postdoc + "' ");
				if (vos != null && vos.length > 0) {
					isJobAbort = true;
				}
			} catch (Exception e) {
	...
	public boolean checkBegindate() throws BusinessException {
		...
		System.out.println("start______asdfsdfsfffffffffffff=======================");
		/***********************************************************************
...
//		if (currow == 0 && CommonValue.HI_PSNDOC_DEPTCHG.equals(strTabCode)) {
		...
		System.out.println("end______asdfsdfsfffffffffffff=======================");
		return true;
	}
	/**
	 * 是否超编_修改任职记录时检查
